...
tap.test('should return standard when trace observer not valid', (t) => {
...
tap.test('should reset/disable trace observer when trace observer not valid', (t) => {
  const stubbedMetrics = createMetricAggregatorStub()
...
tap.test('should generate malformed support metric when trace observer not valid', (t) => {
  const config = Config.initialize({
    feature_flag: {
      infinite_tracing: true
    },
