...
var getAzureInfo = require('../../../lib/azure-info')
...
test('pricing azure info', function(t) {
  t.pass('!!! Skipping these tests until NODE-1518 implemented !!!')
  t.end()
  return
  ...
  fs.readFile(testDirectory + '/azure.json', function readCasefile(err, data) {
    if (err) {
      throw err
    }
  ...
  function testCases(cases) {
    ...
    function timeoutMock(url) {
      ...
      return function wrappedGet(options, callback) {
        setImmediate(function makeRequest() {
          if (url === options) return timeoutCallback()
    ...
    getAzureInfo(agent, function testAWSInfo(info) {
      var expected = AWSCase.expected_vendors_hash && AWSCase.expected_vendors_hash.azure
  ...
  function checkCaching(callback) {
    ...
    getAzureInfo(agent, function getCachedInfo(info) {
