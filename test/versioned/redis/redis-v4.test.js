...
test('Redis instrumentation', async function (t) {
  ...
  await t.test('should log tracking metrics', function(t) {
    ...
    try {
      ...
      const { version } = require('@node-redis/client/package.json')
      assertPackageMetrics({ agent, pkg: '@node-redis/client', version, subscriberType: true })
    }
  })
  ...
  await t.test('should time segments accordingly', function (t, end) {
    const { agent, client } = t.nr
    helper.runInTransaction(agent, async function (tx) {
      assert.ok(tx, 'transaction should be visible')

      const WAIT = 2000
      await client.wait(1, WAIT)
