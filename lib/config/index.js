...
Config.prototype._fromPassed = function _fromPassed(external, internal, arbitrary) {
  ...
  Object.keys(external).forEach(function overwrite(key) {
    ...
    // Handle trace_id_ratio_based root sampler configuration
    if (key === 'distributed_tracing' && this._isTraceIdRatioBasedConfig(external[key])) {
      internal[key] = this._buildTraceIdRatioConfig(external[key])
      return
    }
...
/**
 * Checks if the external config contains a trace_id_ratio_based root sampler configuration
 *
 * @param {object} dtConfig - distributed tracing configuration object from defaultsConfig
 * @returns {boolean} true if it's a trace_id_ratio_based config
 */
Config.prototype._isTraceIdRatioBasedConfig = function _isTraceIdRatioBasedConfig(dtConfig) {
  return dtConfig?.sampler?.root &&
         typeof dtConfig.sampler.root === 'object' &&
         'trace_id_ratio_based' in dtConfig.sampler.root
}

/**
 * Builds the trace_id_ratio_based root sampler configuration structure
 *
 * @param {object} dtConfig - distributed tracing configuration object from defaultsConfig
 * @returns {object} trace_id_ratio_based structured configuration
 */
Config.prototype._buildTraceIdRatioConfig = function _buildTraceIdRatioConfig(dtConfig) {
  return {
    sampler: {
      root: {
        trace_id_ratio_based: {
          ratio: dtConfig.sampler.root.trace_id_ratio_based.ratio
        }
      }
    }
  }
...
/**
 * Assigns the value of the env var as an object to the root sampler in the config.
 *
 * @param {object} params object passed to fn
 * @param {object} params.config agent config
 * @param {string} params.envVar name of environment variable that contains
 * the value for the setting.
 * Example: 'NEW_RELIC_DISTRIBUTED_TRACING_SAMPLER_ROOT_TRACE_ID_RATIO_BASED_RATIO'
 * @param {Function} params.formatter function to coerce env var as they are all strings.
 * This formatter function is defined in the config definition for the params.envVar.
 * Example: float. This will coerce the env var value to a float.
 */
function setTraceIDRatioSampler({ config, envVar, formatter }) {
  const setting = process.env[envVar]
  if (setting) {
    const formattedSetting = formatter(setting)
    config.distributed_tracing.sampler.root = {
      trace_id_ratio_based: {
...
Config.prototype._fromEnvironment = function _fromEnvironment(
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (Object.prototype.hasOwnProperty.call(value, 'default') === true) {
      ...
      // handle custom configuration of the root sampler if it's set to trace_id_ratio_based
      // since root was set to a string but now has to be converted to an object
      if (key === 'root' && config.distributed_tracing.sampler.root === 'trace_id_ratio_based') {
        setTraceIDRatioSampler({
          config,
          envVar: value.trace_id_ratio_based.ratio.env,
          formatter: value.trace_id_ratio_based.ratio.formatter
        })
      }
