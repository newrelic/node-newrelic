 ...
 * the value for the setting. It should look like `a = b` or `{"foo":"bar"}`.
 * @param {Function} params.formatter function to coerce env var
 */
function setTraceIDRatioSampler({ config, envVar, formatter }) {
  ...
  if (setting) {
    ...
    logger.trace({ env: { [envVar]: 'not set' } }, 'environment variable is not valid. Setting default sampler.')
