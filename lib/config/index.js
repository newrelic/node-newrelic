...
Config.prototype._buildTraceIdRatioSamplers = function _buildTraceIdRatioSamplers(key, internal, samplerConfig) {
  ...
  // user can set multiple samplers: 'root', 'remote_parent_sampled', and 'remote_parent_not_sampled'
  for (const sampler of samplers) {
    if (this._isTraceIdRatioBasedConfig(sampler, samplerConfig)) {
      ...
      // where the customer sets `ratioValue` to 0, which is a falsey value.
      if (typeof ratioValue === 'number' && ratioValue >= 0 && ratioValue <= 1) {
        ...
        logger.trace('Setting trace id ratio based sampler on %s', sampler)
      } else {
        logger.error('Not setting trace id ratio based sampler on %s, invalid ratio set.', sampler)
      }
    }
  }
}
