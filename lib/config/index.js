...
Config.prototype._fromPassed = function _fromPassed(external, internal, arbitrary) {
  ...
  Object.keys(external).forEach(function overwrite(key) {
    ...
    if (typeof node === 'object' && !Array.isArray(node) && !(node instanceof RegExp)) {
      ...
      // don't continue iterating if trace_id_ratio_based is in node since we already
      // handled that in this._buildTraceIdRatioSampler call above
      if (!('trace_id_ratio_based' in node)) {
 ...
 * @param {object} samplerConfig - distributed tracing sampler configuration object from defaultsConfig
 */
 ...
 * @param {object} samplerConfig - The distributed tracing sampler configuration object
