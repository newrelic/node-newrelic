 ...
 */
AttributeFilter.prototype.filter = function test(destinations, key) {
  ...
  for (var i = 0; i < DESTINATION_DETAILS.length; ++i) {
    var dest = DESTINATION_DETAILS[i]
    var destId = dest.id
...
AttributeFilter.prototype.update = function update() {
  ...
  DESTINATION_DETAILS.forEach(function forEachDestination(dest) {
    ...
    this._rules[name].include = _importRules(this.config[name].attributes.include)
