...
function AttributeFilter(config) {
  ...
  var updater = _updateRules.bind(null, this)
 ...
 * @private
 *
 * @param {AttributeFilter} filter - The filter to update the cached rules of.
 */
function _updateRules(filter) {
  // If `AttributeFilter#test` becomes memoized, this update function should
  // clear the cached values.

  // Update the global rules.
  filter._rules.global.include = _sortRules(filter.config.attributes.include)
  filter._rules.global.exclude = _sortRules(filter.config.attributes.exclude)

  // And all the destination rules.
  DESTINATIONS.forEach(function forEachDestination(dest) {
    filter._rules[dest].include = _sortRules(filter.config[dest].attributes.include)
    filter._rules[dest].exclude = _sortRules(filter.config[dest].attributes.exclude)
  })
}
