...
function buildSamplers({ config, key, configToUpdate, logger }) {
  ...
  for (const sampler of samplers) {
    if (isAdaptiveSamplingTargetConfig(sampler, samplerConfig)) {
      ...
      if (samplingValue >= 1 && samplingValue <= 120) {
        configToUpdate[key][sampler] = {
          adaptive: {
            sampling_target: int(samplingValue)
          }
        }
        logger.trace('Setting adaptive.sampling_target on %s.%s', key, sampler)
      } else {
        logger.trace('Not setting adaptive.sampling_target on %s.%s as sampling_target value is not valid.', key, sampler)
      }
    }
...
/**
 * Assigns the value of the distributed tracing samplers env var as an adaptive
 * object to the sampler in the config.
 *
 * @param {object} params object passed to fn
 * @param {string} params.key key of the sampler
 ...
 * @param {Logger} params.logger logger instance
 */
function setAdaptiveSamplerFromEnv({ key, config, paths, setNestedKey, logger }) {
  // adaptive sampler is nested in leaf nodes under distributed_tracing > samplers > key
  // so don't continue if path is empty
  if (paths.length === 0) {
    return
  }
  ...
  if (isValidDTSampler(lastPath) && isValidDTSamplerType(key)) {
    ...
    if (setting) {
      ...
      logger.trace('Not setting %s environment variable. Setting %s.%s as `adaptive` with no `sampling_target',
        envVar,
        paths.join('.'),
        key)
