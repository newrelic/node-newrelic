...
function buildAdaptiveSamplers({ config, key, incomingConfig, logger }) {
  if (!isValidDTSampler(key)) {
    return
  }
...
function isAdaptiveSamplingTargetConfig(sampler, samplerConfig) {
  return isValidDTSamplerType(sampler) &&
  // TODO: Have to check for the existence of adaptive.sampling_target
  // not the adaptive string
   typeof samplerConfig[sampler] === 'object' &&
