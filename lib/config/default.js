...
defaultConfig.definition = () => ({
  ...
  transaction_tracer: {
    ...
    /**
     * Sets the time, in milliseconds, for a transaction to trigger an
     * explain plain of the triggering query.
     *
     * Note: as of 2024-07-22, explain plans are not supported by the Node.js
     * agent. Prior to this date, the setting was used to set the threshold
     * for a query being considered "slow." Thereafter, slow queries are
     * governed by the `slow_query_threshold` setting. The value here is used
     * as a fallback to `slow_query_threshold` in order to facilitate a
     ...
     * to be considered for slow query and inclusion in transaction traces.
     */
    slow_query_threshold: {
      formatter: int,
      default: 500,
      env: 'NEW_RELIC_SLOW_QUERY_THRESHOLD'
    }
  },
  /**
