...
const { setSamplersFromEnv } = require('./samplers')

const ENV_PREFIX = 'NEW_RELIC_'

/**
 * Coerces an environment variable's string value to match a schema node's type.
 * Environment variables are always strings; this converts them to the type the
 * schema declares. Types the conversion can't unambiguously handle (strings,
 * unions, oneOf, etc.) pass the raw string through.
 *
 * @param {string} rawValue The environment variable value.
 * @param {object} schemaNode The setting's schema node.
 * @returns {*} The coerced value.
 */
function coerceValue(rawValue, schemaNode) {
 ...
 * @param {object} [deps.logger] An optional logger for unrecognized variables.
...
function applyEnvironmentOverrides(
  ...
  for (const name of Object.keys(env)) {
    ...
    setAtPath(
      ...
      coerceValue(rawValue, resolution.node)
    )
