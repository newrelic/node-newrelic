...
class Aggregator extends EventEmitter {
  ...
  reconfigure(config) {
    ...
    const eventHarvestSettings = config.event_harvest_config
    if (eventHarvestSettings) {
      const limit = eventHarvestSettings.harvest_limits[this.method]
      if (limit) {
        this.periodMs = eventHarvestSettings.report_period_ms
        this.limit = limit
      } else {
