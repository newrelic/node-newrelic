...
exports.getKeys = function getKeys(data, keys) {
  ...
  for (var i = 0; i < keys.length; ++i) {
    var key = keys[i]
    if (!properties.hasOwn(data, key)) {
      logger.debug('Key %s missing from metadata', key)
      return null
    }

    // Each value can only be up to 255 bytes. Since we are dealing with UTF-8,
    // each character could be between 1 and 4 bytes long. In order to prevent
    // broken characters we must pop characters off the end, one at a time,
    // until we are below the target length.
    var value = data[key].substr(0, 255)
    while (value && Buffer.byteLength(value) > 255) {
