...
module.exports.getBootId = function getBootId(agent, callback) {
  ...
  common.readProc('/proc/sys/kernel/random/boot_id', function getBootId(data) {
    var dockerError = agent.metrics.getOrCreateMetric(NAMES.UTILIZATION.BOOT_ID_ERROR)
    if (!data) {
...
function fetchDockerVendorInfo(agent, callback) {
  ...
  common.readProc('/proc/self/cgroup', function getCGroup(data) {
    if (!data) {
      return callback(null)
    }
