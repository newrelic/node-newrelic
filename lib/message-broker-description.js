...
class MessageBrokerDescription {
  ...
  // These names are used in segment naming. They are derived from `Transport`,
  // the source of truth for distributed-trace transport values.
  static TRANSPORT_TYPE_AMQP = Transport.AMQP
  static TRANSPORT_TYPE_IRONMQ = Transport.IRONMQ
  static TRANSPORT_TYPE_KAFKA = Transport.KAFKA
  static TRANSPORT_TYPE_RABBITMQ = Transport.AMQP
  /**
   * The `Queue` type is used for generic queue transports, e.g. SQS.
   * @type {string}
   */
  static TRANSPORT_TYPE_QUEUE = Transport.QUEUE
