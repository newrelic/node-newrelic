...
const { calculateCallbackTokens } = require('./utils')
...
class LlmEmbedding extends LlmEvent {
  ...
  getTotalTokens() {
    const totalToken =
      this.bedrockResponse?.usage?.total_tokens || this.bedrockResponse?.usage?.totalTokens ||
      this.bedrockResponse?.headers['x-amzn-bedrock-total-token-count']
