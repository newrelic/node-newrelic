...
class LlmChatCompletionMessage extends LlmEvent {
  ...
  setTokenCount(agent, command, response) {
    ...
    if (tokenCB) {
      ...
      const promptToken = calculateCallbackTokens(tokenCB, command.modelId, promptContent)
      const completionToken = calculateCallbackTokens(tokenCB, command.modelId, completionContent)
