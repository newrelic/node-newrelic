...
function setUsageTokens(response, context) {
  ...
  if (tokenUsageHeadersExist(response)) {
    ...
    context['response.usage.total_tokens'] =
      response?.headers['x-amzn-bedrock-total-token-count'] || getTotalTokenCount(context)
  }
}

function tokenUsageAttributesExist(response) {
  const tokensExistA = response?.usage?.input_tokens && response?.usage?.output_tokens
  const tokensExistB = response?.usage?.inputTokens && response?.usage?.outputTokens
