...
function usageTokens(response, context) {
  // all 3 usage attributes must be available in order to add all usage attributes to response
  if (tokenUsageAttributesExist(response)) {
    context['response.usage.prompt_tokens'] = response.usage.input_tokens || response.usage.inputTokens
    context['response.usage.completion_tokens'] = response.usage.output_tokens || response.usage.outputTokens
    context['response.usage.total_tokens'] = response.usage.total_tokens || response.usage.totalTokens
  } else if (tokenUsageHeadersExist(response)) {
    // TODO: not sure if x-amzn-bedrock-total-token-count is an actual header
    context['response.usage.prompt_tokens'] = response.headers['x-amzn-bedrock-input-token-count']
    context['response.usage.completion_tokens'] = response.headers['x-amzn-bedrock-output-token-count']
    context['response.usage.total_tokens'] = response.headers['x-amzn-bedrock-total-token-count']
  }
}

function tokenUsageAttributesExist(response) {
  return ((response?.usage?.input_tokens && response?.usage?.output_tokens && response?.usage?.total_tokens) ||
     (response?.usage?.inputTokens && response?.usage?.outputTokens && response?.usage?.totalTokens))
}

// TODO: confirm if these are correct headers
function tokenUsageHeadersExist(response) {
  return ((response?.headers['x-amzn-bedrock-input-token-count'] && response?.headers['x-amzn-bedrock-output-token-count'] &&
      response?.headers['x-amzn-bedrock-total-token-count']))
}
