...
function setUsageTokens(response, context) {
  ...
  if (tokenUsageAttributesExist(response)) {
    const promptTokens = Number(response?.usage?.input_tokens || response?.usage?.inputTokens)
    const completionTokens = Number(response?.usage?.output_tokens || response?.usage?.outputTokens)
