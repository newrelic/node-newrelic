...
module.exports = class LlmChatCompletionSummary extends LlmEvent {
   ...
   * @param {number} params.timeToFirstToken Duration in milliseconds between when the request was issued and the first token was received. This attribute **MUST** only be captured for streaming requests.
  ...
  constructor({ agent, segment, transaction, vendor, responseModel, requestModel, requestId,
    responseOrg, temperature, maxTokens, numMsgs = 0, finishReason, timeToFirstToken, error }) {
    ...
    if (timeToFirstToken) this['time_to_first_token'] = timeToFirstToken
