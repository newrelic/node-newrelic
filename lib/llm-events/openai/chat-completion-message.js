...
module.exports = class LlmChatCompletionMessage extends LlmEvent {
  constructor({
    ...
    if (agent.config.ai_monitoring.record_content.enabled === true) {
      ...
      if (this.is_response && response?.choices?.[0]?.finish_reason === 'tool_calls') {
        // `content` is null on outgoing tool calls, and OpenAI doesn't
        // know the result yet. For a better UI experience, we will
        // provide a description of the tool being called and assign
        // it to `content`.
        const toolCall = response?.choices?.[0]?.message?.tool_calls?.[0]
        const toolArgs = JSON.parse(toolCall?.function?.arguments || '{}')
        this.content = `Tool call: ${toolCall?.function?.name}(${JSON.stringify(toolArgs)})`
      }
    }
