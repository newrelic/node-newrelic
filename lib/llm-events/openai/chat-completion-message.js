...
module.exports = class LlmChatCompletionMessage extends LlmEvent {
  constructor({
    ...
    this.setTokenCount(agent, request, response, content)
  }

  setTokenCount(agent, request, response, content) {
    const tokenCB = agent.llm?.tokenCountCallback

    if (tokenCB) {
      const promptContent = content
      const completionContent = response?.choices?.[0]?.message?.content || response?.output?.[0]?.content?.[0]?.text
