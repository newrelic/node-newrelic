...
for (const _level of Object.keys(LEVELS)) {
  ...
  /**
   * Writes a log message if audit logging or trace logging is enabled.
   * We consider trace level logging equivalent to audit logging. But audit
   * logging can happen at _any_ log level greater than or equal to 'info'
   * level.
   *
   * @param {*} args The same arguments that would be passed to any level
   * named logging function.
   *
   * @returns {*} The result of the logging function or `undefined`.
   */
  ...
  /**
   * Indicates is audit logging, or trace level logging, is enabled for
   * the current logger. This is helpful when an audit log is possible, but
   * there may be expensive operations needed for the log that do not need
   * to be run when audit logging is disabled.
   *
   * @returns {boolean} True if a log will be written. False otherwise.
   */
  loggingFunctions.auditEnabled = function auditEnabled() {
    return this.options.auditLogging === true || this.traceEnabled() === true
  }
}
  ...
  for (let i = 0, l = args.length; i < l; ++i) {
    if (typeof args[i] === 'function') {
      ...
      try {
        ...
        this.debug('Failed to stringify object for log')
