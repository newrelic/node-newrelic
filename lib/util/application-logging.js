...
utils.createFeatureUsageMetrics = function createFeatureUsageMetrics(agent) {
  ...
  metrics
    .getOrCreateMetric(
      `${LOGGING.METRICS}${config.application_logging.metrics.enabled ? 'enabled' : 'disabled'}`
    )
    .incrementCallCount()
  metrics
    .getOrCreateMetric(
      `${LOGGING.FORWARDING}${
        config.application_logging.forwarding.enabled ? 'enabled' : 'disabled'
      }`
    )
    .incrementCallCount()
  metrics
    .getOrCreateMetric(
      `${LOGGING.LOCAL_DECORATING}${
        config.application_logging.local_decorating.enabled ? 'enabled' : 'disabled'
      }`
    )
    .incrementCallCount()
  metrics
    .getOrCreateMetric(
      `${LOGGING.LABELS}${
        config.application_logging.forwarding.labels.enabled ? 'enabled' : 'disabled'
      }`
    )
    .incrementCallCount()
}
