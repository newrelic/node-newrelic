...
function encode(bytes, keyBytes) {
  for (var i = 0; i < bytes.length; i++) {
    // This is really dense but happens commonly so I'm inlining some of what
    // could be tossed into variables. It takes the current byte of bytes, then
    // XORs it with the current byte of the key (which uses modolo to make sure
    // to not overrun the end.)
    bytes.writeUInt8(bytes.readUInt8(i) ^ keyBytes.readUInt8(i % keyBytes.length), i);
  }
  return bytes;
}
// TODO: cat-test There will be a cross agent test added for this and the next,
// it was generated from the output of the ruby agent, which has had this
// obfusticator for a long time and can be considered a trusted quantity. For
// now use test/lib/obfuscation-data.json which will be merged into cross agent
// tests eventually.
...
// TODO: cat-test This can be directly tested using cross_agent_tests
// cat_map.json. You don't need to use the whole tests for all of it, but you
// can use them as input/output combinations for subsets (making sure the
// hashing happens correctly)
...
function getHash(appName, txName) {
  ...
  md5sum.update(appName + ';' + txName);
  var buf = new Buffer(md5sum.digest('base64'), 'base64');
  // pull the low 4 bytes in network byte order
  var low4 = buf.slice(-4, buf.length).readUInt32BE(0);
  // pull the low 31 bits out of those 4 bites and return it.
  return low4;
}
