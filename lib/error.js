...
function createError(transaction, exception, customParameters, config) {
  ...
  if (typeof exception === 'string') {
    ...
    if (exception && exception.name ||
        (exception.constructor && exception.constructor.name)) {
  ...
  var i, l
  if (transaction) {
    var custom = transaction.trace.custom
  ...
  if (transaction && transaction.url) {
    ...
    if (config.capture_params) {
      var reqParams = transaction.trace.parameters
  ...
  var res = [timestamp, name, message, type, params]
  if (transaction) {
...
ErrorTracer.prototype.removeErrorsForTransaction = removeErrorsForTransaction

function removeErrorsForTransaction(transaction) {
  if (!transaction) return
  var transaction_id = transaction.id
  for (var i = this.errors.length; i >= 0; --i) {
    var error = this.errors[i]
    if (error && error.transaction === transaction_id) {
      this.errors.splice(i, 1)
      this.errorCount--
      this.seen.splice(this.seen.indexOf(error), 1)
    }
  }
}
 ...
 * little as possible.
 */
ErrorTracer.prototype.onTransactionFinished = onTransactionFinished
...
function onTransactionFinished(transaction, metrics) {
  ...
  if (transaction.ignore) return this.removeErrorsForTransaction(transaction)
