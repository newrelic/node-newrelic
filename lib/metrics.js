...
Metrics.prototype.externalMetrics = function (host, library) {
  if (!host) {
    logger.debug('External request metrics need to be associated with a host. Not measuring.');
    return function noop() {};
  }

  var self = this;
  return function (probe, scope) {
    var duration = probe.getDurationInMillis();

    self.measureDurationUnscoped('External/all', duration);

    var rollupName = 'External/all';
    if (probe.trace.transaction.isWeb()) {
      rollupName += '/Web';
    }
    else {
      rollupName += '/Other';
