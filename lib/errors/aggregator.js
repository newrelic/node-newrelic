...
const errorUtils = require('../util/errors')
...
class ErrorAggregator extends EventAggregator {
  ...
  onTransactionFinished(transaction, metrics) {
    ...
    if (transaction.exceptions.length && !isIgnoredErrorStatusCode) {
      for (let i = 0; i < transaction.exceptions.length; i++) {
        ...
        if (
          this._collect(transaction, exception[0], exception[1], exception[2]) &&
          !errorUtils.isExpectedException(transaction, exception[0], this.config, urltils)
        ) {
          ++collectedErrors
        }
      }
  ...
  _collect(transaction, exception, customAttributes, timestamp) {
    ...
    // gross: if the error ends up being expected, decrement the count
    // so it's not counted
    if( true === error[4].agentAttributes['error.expected'] ) {
      this.errorCount--
      if (transaction) {
        if (transaction.isWeb()) {
          this.webTransactionErrorCount--
