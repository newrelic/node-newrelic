...
function _collect(transaction, exception, customParameters, timestamp) {
  ...
  if (this.errors.length < MAX_ERRORS) {
    ...
    // XXX: 2016-05-24 Remove this when APM UI is updated to use correct `request.uri`
    //
    // For right now, when this flag is enabled, the `request.uri` will be added
    ...
    if (this.config.feature_flag.send_request_uri_attribute) {
      ...
      delete err[4].agentAttributes['request.uri']
