...
function createError(transaction, exception, customParameters, config) {
  ...
  if (transaction) {
    ...
    if (transaction.isWeb()) {
      params['request.uri'] = transaction.getScrubbedUrl()
    }

    // Copy all of the parameters off of the transaction.
    params.agentAttributes = transaction.trace.attributes.get('error_collector')
    params.intrinsics = transaction.getIntrinsicAttributes()

    // Custom params aren't filtered by attributes.enabled or attributes.exclude, just by
    // high security mode.
    if (!config.high_security) {
      var custom = transaction.trace.custom.get('error_collector')
      urltils.overwriteParameters(config, custom, params.userAttributes)
    }
  }
