...
function createError(transaction, exception, customAttributes, config) {
  let {name, message, type} = errorUtils.extractErrorInformation(
    transaction,
    exception,
    config,
    urltils
  ...
  if (
    errorUtils.isExpectedErrorMessage(config, message) ||
    errorUtils.isExpectedErrorClass(config, type)
  ) {
    params.agentAttributes['error.expected'] = true
  }
