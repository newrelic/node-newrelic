...
function createError(transaction, exception, customAttributes, config) {
  ...
  if (transaction) {
    ...
    params.intrinsics = transaction.getIntrinsicAttributes(false)
...
function _getErrorEventIntrinsicAttrs(transaction, errorClass, message, timestamp, conf) {
  ...
  if (transaction) {
    ...
    if (transaction.agent.config.feature_flag.distributed_tracing) {
      transaction._addDistributedTraceIntrinsics(attributes, false)
    } else {
