...
module.exports = {
  ...
  shouldIgnoreError: function shouldIgnoreError(transaction, exception, config) {
    ...
    if (this.shouldIgnoreErrorClass(errorInfo, config)) {
      return true
    }

    if (this.shouldIgnoreErrorMessage(errorInfo, config)) {
      return true
    }
  ...
  shouldIgnoreErrorClass: function shouldIgnoreErrorClass(errorInfo, config) {
    if (!config.error_collector.expected_classes.length < 1) {
      return false
    }
    return -1 !== config.error_collector.ignore_classes.indexOf(errorInfo.type)
  },

  shouldIgnoreErrorMessage: function shouldIgnoreErrorClass(errorInfo, config) {
