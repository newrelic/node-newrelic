...
class ErrorCollector {
  ...
  onTransactionFinished(transaction) {
    ...
    if (transaction.userErrors.length) {
      for (let i = 0; i < transaction.userErrors.length; i++) {
        ...
        if (this._collect(transaction, exception)) {
          ++collectedErrors
        }
      }
    }
   ...
   */
  addUserError(transaction, exception) {
    if (!exception) return
    ...
    if (transaction) {
      ...
      this._collect(transaction, exception.error, exception.customAttributes, exception.timestamp)
    }
  }

  /**
   * Wrapper for _collect, including logic for whether to ignore an error. Exists to allow
   * userErrors to bypass ignore logic. NOTE: this interface is unofficial and may change in future.
   *
   * @param  {Transaction}  transaction  Transaction associated with the error.
  ...
  collect(transaction, exception) {
    ...
    return this._collect(transaction, exception)
  }
   ...
   * @return  {bool}  True if the error was collected.
   */
  _collect(transaction, exception) {
    const error = exception.error
    if (error) {
      if (this._haveSeen(transaction, error)) {
        return false
      }
    ...
    if (!error && (!transaction || !transaction.statusCode || transaction.error)) {
      return false
    }
