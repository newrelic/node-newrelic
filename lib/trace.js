...
function Transaction(agent) {
    ...
    function finished(tracer) {
        ...
        try {
            ...
            if (self.url) {
                self.scope = metrics.recordWebTransactionMetrics(agent.getMetricNormalizer(), unscopedStats, 
...
function Tracer(transaction, metricNameOrCallback) {
    ...
    this.appendToStack = good ? 
    	function(error) {
        	this._parent = appendToStackAndFindParent(this, error);
    	} : noop;
    
    this.popFromTransaction = good ? 
		function() { 
...
Tracer.prototype.getExclusiveDurationInMillis = function() {
    return Math.max(0, this.getDurationInMillis() - this.childDurationInMillis);
};
