  ...
  start() {
    for (const aggregator of this.aggregators) {
      if (aggregator.enabled && aggregator.delay > 0) {
        ...
        aggregator.delayTimeout = setTimeout(() => {
          aggregator.start()
        }, aggregator.delay)
        aggregator.delayTimeout.unref()
      ...
      if (aggregator.enabled && aggregator.duration > 0) {
        ...
        aggregator.durationTimeout = setTimeout(() => {
          aggregator.stop()

          // for profiling aggregator, we want to disable profiling after duration ends so that it doesn't
          // continue to collecting data on soft restarts
          if (aggregator.method === 'pprof_data') {
            aggregator.agent.config.profiling.enabled = false
          }
        }, aggregator.delay + aggregator.duration)
        aggregator.durationTimeout.unref()
      }
    }
  }
