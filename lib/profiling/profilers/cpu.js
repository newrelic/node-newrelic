...
class CpuProfiler extends BaseProfiler {
  ...
  start() {
    ...
    this.#pprof.time.start({
      ...
      // add a measured on-CPU (`cpu`) nanoseconds value to each sample
      // alongside `wall`. cpu is the on-CPU time the native profiler
      // measures per context, so cpu <= wall by definition.
      collectCpuTime: true,
