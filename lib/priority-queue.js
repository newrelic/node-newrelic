...
PriorityQueue.prototype.getRawEvents = function getRawEvents() {
  ...
  if (min) {
    getRawEvents(min, events)
  }

  return events

  function getRawEvents(head, evts) {
    var current = head

    do {
      evts.push({ value: current.value, priority: current.key })
      if (current.child) {
        getRawEvents(current.child, evts)
      }
...
PriorityQueue.prototype.merge = function merge(events) {
  ...
  if (!(queue instanceof PriorityQueue)) {
    queue = new PriorityQueue(events.length)
    events.forEach(function add(e) {
      queue.add(e.value, e.priority)
    })
  }

  this._data.union(queue._data)
}
