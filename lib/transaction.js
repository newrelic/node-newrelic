...
function Transaction(agent) {
  ...
  this.agent = agent;
  this.metrics = new Metrics(agent.config.apdex_t, agent.mapper);
...
Transaction.prototype._setApdex = function (name, duration) {
  ...
  if (urltils.isError(statusCode) && !errorTracer.ignoreStatusCode(statusCode)) {
