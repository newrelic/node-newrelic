...
Transaction.prototype.measureWeb = function (requestURL,
  ...
  if (name && name.ignore) {
    ...
    metrics.measureMilliseconds(NAMES.WEB,  null, duration);
    metrics.measureMilliseconds(NAMES.HTTP, null, duration);
    ...
    metrics.measureMilliseconds(this.scope, null, duration, maxDuration);

    [NAMES.APDEX + '/' + partialName, NAMES.APDEX].forEach(function (name) {
