...
Transaction.prototype.measureWeb = function (requestURL,
  ...
  if (statusCode === 414 || // Request-URI Too Long
    ...
    if (name.normalized) {
      ...
      partialName = NAMES.URI + this.url;
    }
  }
  ...
  if (name && name.ignore) {
    ...
    this.scope = NAMES.WEB + '/' + partialName;

    this.metrics.measureMilliseconds(NAMES.WEB,  null, duration, maxDuration);
    this.metrics.measureMilliseconds(NAMES.HTTP, null, duration, maxDuration);
    this.metrics.measureMilliseconds(this.scope, null, duration, maxDuration);

    this._setApdex(NAMES.APDEX + '/' + partialName, duration);
    this._setApdex(NAMES.APDEX,                     duration);
  }
