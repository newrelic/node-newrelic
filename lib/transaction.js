 ...
 */
Transaction.prototype.setScope = function (requestURL, statusCode) {
  this.url = web.scrubURL(requestURL);
  this.statusCode = statusCode;

  if (web.isError(statusCode)) {
    // we've got an error transaction
    this.partialScope = NAMES.STATUS + statusCode;
  }
  else if (!this.partialScope) {
    /* Nothing's already come up with a name for this transaction, so normalize
     * and potentially apply the URL backstop now.
     */
    this.partialScope = this.agent.normalizer.normalize(this.url);
  }

  // the normalizer returns no name if a transaction should be ignored
  if (this.partialScope) {
    this.scope = NAMES.WEB + '/' + this.partialScope;
  }
  else {
    this.ignore = true;
  }
