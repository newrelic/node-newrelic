...
module.exports = class BulkSubscriber extends DbSubscriber {
  constructor({ agent, logger }) {
    ...
    for (const method of CURSOR_METHODS) {
      ...
      this.#wrapMethod(bulkOperation, method, collection.collectionName)
    }
  ...
  #wrapMethod(instance, methodName, collection) {
    ...
    instance[methodName] = function nrWrappedMethod(...args) {
      ...
      const capturedOperation = instance.constructor.name === 'OrderedBulkOperation'
        ? 'orderedBulk'
        : 'unorderedBulk'
      ...
      ctx = self.createSegment({
        name: `${DB_METRIC_CONSTANTS.STATEMENT}/${self.system}/${collection}/${capturedOperation}/batch`,
