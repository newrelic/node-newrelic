...
const ctxSym = Symbol.for('nr.mongo.ctx')
 ...
 * `MongoClient`, the `database_name` field will be omitted. You must acquire
...
module.exports = function getHostDetails(mongoObject) {
  ...
  if (mongoObject[ctxSym]) {
    const ctx = mongoObject[ctxSym]
    databaseName = ctx.databaseName
    host = ctx.host
    port = ctx.port
  } else if (mongoObject.constructor.name === 'MongoClient') {
