 ...
 * @param {object} params.logger instance of logger
 */
function addDTHeaders({ transaction, config, request, logger }) {
 ...
 * @param {object} params.request undici request object
 */
function requestCreateHook({ request }) {
  ...
  try {
    ...
    addDTHeaders({ transaction, config: this.config, request, logger: this.logger })
  } catch (err) {
 ...
 */
function requestHeadersHook({ response }) {
 ...
 */
function endAndRestoreSegment({ error }) {
  const config = this.config
  const agent = this.agent
  const context = agent.tracer.getContext()
  const activeSegment = context?.extras?.undiciSegment
  ...
  if (error && tx && config.feature_flag.undici_error_tracking === true) {
    handleError({ agent, tx, error, logger: this.logger })
  }
 ...
 * @param {Error} params.error error from undici request
 * @param {Logger} params.logger agent logger
 */
function handleError({ agent, tx, error, logger }) {
