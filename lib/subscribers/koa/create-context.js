...
class KoaCreateContextSubscriber extends Subscriber {
  constructor({ agent, logger, packageName = 'koa' }) {
    ...
    if (!statusDescriptor?.get || !statusDescriptor?.set) {
      this.logger.debug('Failed to find status descriptor on ctx.response, skipping status wrap')
      return
    }
