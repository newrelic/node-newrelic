...
module.exports = class AzureHandler {
  ...
  async handle({ thisArg, originalHandler, handlerArgs }) {
    ...
    // If the handler throws (or rejects), we must still record the error and end
    // the transaction before re-throwing. Otherwise transaction.end() never runs,
    // so record() and logs.flush() never fire and the entire invocation — its
    // transaction, spans, error, and in-context logs — is silently dropped. This
    // matters most for non-HTTP triggers (e.g. Service Bus), where a thrown error
    // is the normal failure signal (abandon → retry → dead-letter). The error path
    // ends the transaction directly rather than via finalizeTransaction(), since
    // subclass overrides (e.g. HTTP) assume a successful `result` to finalize with.
    const wrappedHandler = async (...args) => {
      try {
        ...
        this.handleColdStart(transaction)
        this.agent.errors.add(transaction, err)
        transaction.end()
        throw err
      }
    }
