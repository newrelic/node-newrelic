...
class GoogleGenAIGenerateContentStreamSubscriber extends GoogleGenAIGenerateContentSubscriber {
  ...
  handler(data, ctx) {
    if (!this.agent?.config?.ai_monitoring?.streaming?.enabled) {
      this.logger.debug('config.ai_monitoring.streaming.enabled is set to false, not creating segment.')
      return ctx
    }
  ...
  asyncEnd(data) {
    ...
    if (!this.agent?.config?.ai_monitoring?.enabled) {
      this.logger.debug('config.ai_monitoring.enabled is set to false, not recording Llm events.')
      return
    }
    if (!this.agent?.config?.ai_monitoring?.streaming?.enabled) {
      this.logger.warn(
