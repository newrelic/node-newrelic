...
class MiddlewareWrapper {
   ...
   * @param {boolean} params.isHandler flag to indicate the middleware is the final one in a router layer stack
   * @returns {Function} a wrapped function used to record a segment and assign necessary transaction data
   */
  wrap({ handler, prefix, route, segmentName, nextIdx = -1, isHandler = false }) {
    const self = this
    ...
    function wrappedHandler (...args) {
      ...
      self.wrapDoneHandler({ segment, ctx, args, handler, txInfo, route, nextIdx, isHandler })
