...
class MiddlewareWrapper {
   ...
   * @param {boolean} params.appendToPath if false, skips appending route to nameState and passes route directly to constructRecorder instead
   * @returns {Function} a wrapped function used to record a segment and assign necessary transaction data
   */
  wrap({ handler, prefix, route, segmentName, nextIdx = -1, appendToPath = true }) {
    const self = this
    ...
    function wrappedHandler (...args) {
      ...
      if (route && !errorWare && appendToPath) {
      ...
      const recorder = self.constructRecorder({ handler, transaction, segmentName, route: appendToPath ? undefined : route })
   ...
   * @param {string} params.segmentName predefined segment name, short circuits composable naming
   * @param {string} [params.route] route to use directly instead of reading from nameState
   * @returns {Function | null} a middleware recorder function or null
   */
  constructRecorder({ handler, transaction, segmentName, route }) {
    ...
    const fnName = getFunctionName(handler)
    const path = route ?? transaction.nameState.getPath() ?? '/'
