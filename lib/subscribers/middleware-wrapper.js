...
class MiddlewareWrapper {
   ...
   * @param {boolean} params.asyncDoneHandler flag to indicate if the next/done handler is async(koa handlers are async but defined without `async` keyword)
   * @param {boolean} params.isHandler flag to indicate if middleware is coming from a router with multiple middleware
   * @returns {Function} a wrapped function used to record a segment and assign necessary transaction data
   */
  wrap({ handler, prefix, route, segmentName, nextIdx = -1, asyncDoneHandler = false, isHandler = false }) {
    const self = this
    ...
    function wrappedHandler (...args) {
      ...
      self.wrapDoneHandler({ segment, ctx, args, handler, txInfo, route, nextIdx, asyncHandler: asyncDoneHandler, isHandler })
