...
class MiddlewareWrapper {
   ...
   * @param {boolean} params.popPathOnExit if true, pops the route from the path stack after the handler returns
   * @returns {Function} a wrapped function used to record a segment and assign necessary transaction data
   */
  wrap({ handler, prefix, route, segmentName, nextIdx = -1, popPathOnExit = false }) {
    const self = this
      ...
      try {
        ...
        if (result?.then) {
          return result.then(function onThen(val) {
            if (popPathOnExit && route) transaction.nameState.popPath(route)
            segment.touch()
            return val
          },
