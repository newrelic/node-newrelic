...
class MiddlewareWrapper {
  constructor({ agent, logger, system, errorHandler }) {
    ...
    this.als = agent.tracer._contextManager._asyncLocalStorage
  ...
  wrap({ handler, prefix, route, segmentName, nextIdx = -1 }) {
    ...
    function wrappedHandler (...args) {
      ...
      try {
        const result = self.als.run(newCtx, Reflect.apply, handler, this, args)
        if (result?.then) {
    ...
    if (isSync) {
      ...
      function wrappedDone() {
        ...
        return als.run(ctx, () => {
          ...
          segment.touch()
          return Reflect.apply(doneFn, doneThis, doneArgs)
        })
      }
      if (nextIdx === -1) {
