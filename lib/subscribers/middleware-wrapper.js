...
class MiddlewareWrapper {
   ...
   * @param {string} [params.eventName] lifecycle event name (e.g. 'onRequest') — used for naming only, never appended to nameState
   * @returns {Function} a wrapped function used to record a segment and assign necessary transaction data
   */
  wrap({ handler, prefix, route, segmentName, nextIdx = -1, eventName }) {
    const self = this
    ...
    function wrappedHandler (...args) {
      ...
      if (route && !errorWare && !eventName) {
        transaction.nameState.appendPath(route, request.params)
      }

      const name = self.nameSegment({ handler, prefix, route, errorWare, segmentName, eventName })
      const recorder = self.constructRecorder({ handler, transaction, segmentName, eventName })
   ...
   * @param {string} [params.eventName] lifecycle event name, takes precedence over route for naming
   * @returns {string} name of segment for given middleware function
   */
  nameSegment({ handler, prefix = this.prefix, route, errorWare, segmentName, eventName }) {
    if (segmentName) {
      return segmentName
    }
    ...
    if (eventName) {
      return `${prefix}/${fnName}//${eventName}`
    }
   ...
   * @param {string} params.segmentName predefined segment name, short circuits composable naming
   * @param {string} [params.eventName] lifecycle event name used directly in metric name instead of nameState path
   * @returns {Function | null} a middleware recorder function or null
   */
  constructRecorder({ handler, transaction, segmentName, eventName }) {
    ...
    const fnName = getFunctionName(handler)
    const path = eventName ? `/${eventName}` : transaction.nameState.getPath() ?? '/'
