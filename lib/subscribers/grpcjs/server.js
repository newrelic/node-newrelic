...
module.exports = class GrpcServerSubscriber extends Subscriber {
  ...
  handler(data, ctx) {
    ...
    _arguments[1] = function wrappedGrpcHandler(...args) {
      let transaction = self.agent.tracer.getTransaction()
      if (transaction != null) {
      ...
      const ctx = self.#createContext(self, handlerName, handlerType)
  ...
  #createContext(instance, handlerName, handlerType) {
    ...
    if (transaction == null) {
      // It should always be the case that the context's transaction field is
      // empty. But we don't make any assumptions around here.
    ...
    transaction.baseSegment = ctx.segment
