...
/**
 * A message consumer is typically 2 fold:
 *  call consume to schedule consumption, when messages are enqueued, there is a callback
 *  to consume the messages. A segment should be created when call to consume is invoked if there is an active transaction. For every message consumption, it creates a transaction and accepts headers accordingly
 */
class MessageConsumer extends Subscriber {
  ...
  addConsumeParameters(tx) {
    ...
    if (this.isString(this.queue) && this.config.high_security === false) {
