...
const ClusterConstructorSubscriber = require('./cluster-constructor.js')
...
module.exports = class ConstructorSubscriber extends Subscriber {
  ...
  /**
   * Captures the `cluster_id` for a newly created producer/consumer, if
   * cluster metrics are enabled. The `kafkajs.Cluster` instance backing
   * `instance` is picked up from `ClusterConstructorSubscriber.pendingCluster`
   * (set by that subscriber immediately prior, synchronously, as part of the
   * `kafkajs.Client#producer`/`#consumer` call). Its `brokerPool` is
   * then wrapped so that the first time it resolves a metadata refresh
   * (something `kafkajs` already does on its own, e.g. on `send()`/
   * `subscribe()`), the resolved `clusterId` is cached on the shared
   * `kafkaCtx` and reused from then on. The wrapper removes itself once the
   * value has been captured.
   *
   * @param {object} instance Producer or consumer client instance.
   */
  #trackClusterId(instance) {
    const cluster = ClusterConstructorSubscriber.pendingCluster.current
    ClusterConstructorSubscriber.pendingCluster.current = null

    if (this.agent.config.kafka.metrics.cluster.metrics.enabled !== true) {
      return
    }
    if (!cluster?.brokerPool || typeof cluster.brokerPool.refreshMetadata !== 'function') {
      return
    }

    const kafkaCtxObj = instance[kafkaCtx]
    const brokerPool = cluster.brokerPool
    const origRefresh = brokerPool.refreshMetadata.bind(brokerPool)
    brokerPool.refreshMetadata = async function nrRefreshMetadata(...args) {
      const result = await origRefresh(...args)
      kafkaCtxObj.clusterId = brokerPool.metadata?.clusterId
      if (kafkaCtxObj.clusterId !== undefined) {
        brokerPool.refreshMetadata = origRefresh
      }
      return result
    }
  }
