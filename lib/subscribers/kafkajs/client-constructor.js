...
module.exports = class ConstructorSubscriber extends Subscriber {
  ...
  #refreshAndRecordProduceMetrics(instance, batch, data) {
    const cid = getClusterIdFromCache(this.#clusterIdCache, instance[kafkaCtx]?.brokers)
    if (!cid && instance[kafkaCtx]?._kafkaClient) {
      fetchAndCacheClusterId(
        this.#clusterIdCache,
        this.#clusterIdInFlight,
        instance[kafkaCtx]._kafkaClient,
        instance[kafkaCtx].brokers
      )
        .then((id) => { if (id && instance[kafkaCtx]) instance[kafkaCtx].clusterId = id })
        .catch(() => {})
    }
    const effectiveCid = cid ?? instance[kafkaCtx]?.clusterId
    if (effectiveCid) {
      recordClusterProduceMetrics(this.agent.metrics, effectiveCid, batch, data)
    }
  }
