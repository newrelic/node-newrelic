...
const { getClusterIdFromCache, _fetchAndCacheClusterId } = require('./utils/cluster-id-cache.js')
  ...
  #wrapRunForBatches(instance) {
    ...
    instance.run = function nrWrappedRunForBatches(...args) {
      ...
      args[0].eachBatch = function nrWrappedEachBatch() {
        ...
        const clusterId = instance[kafkaCtx]?.clusterId ?? getClusterIdFromCache(instance[kafkaCtx]?.brokers)
        if (clusterId) {
          const m = self.agent.metrics.getOrCreateMetric(
            `MessageBroker/Kafka/Cluster/${clusterId}/Topic/${batch.topic}/Consume`
          )
          for (let i = 0; i < batch.messages.length; i++) m.incrementCallCount()
        }
