...
module.exports = class ConstructorSubscriber extends Subscriber {
  ...
  /**
   * Wraps the `kafkajs.Client`'s private `createCluster` factory (reached via
   * the well-known `Symbol('private:Kafka:createCluster')` it assigns onto
   * itself) so that we can observe every `Cluster` this client constructs --
   * one is created per `.producer()`/`.consumer()`/`.admin()` call, and
   * `cluster_id` is never exposed anywhere on the `Client`, `producer`, or
   * `consumer` objects themselves. Once we have a `Cluster` reference, its
   * `brokerPool.refreshMetadata` is wrapped so that whenever `kafkajs`
   * performs a metadata refresh of its own accord (e.g. on `send()`/
   * `subscribe()`), the resolved `clusterId` is cached on the client's shared
   * `kafkaCtx`.
   *
   * @param {object} client The `kafkajs.Client` instance.
   */
  #trackClusterId(client) {
    ...
    for (const symbol of symbols) {
      if (symbol.toString() !== 'Symbol(private:Kafka:createCluster)') {
        continue
      }

      const origCreateCluster = client[symbol]
      client[symbol] = function wrappedCreateCluster(...args) {
        const cluster = origCreateCluster.apply(this, args)
        const brokerPool = cluster.brokerPool
        const origRefresh = brokerPool.refreshMetadata
        brokerPool.refreshMetadata = async function wrappedRefreshMeta(...args) {
          const result = await origRefresh.apply(this, args)
          client[kafkaCtx].clusterId = this.metadata?.clusterId
          return result
        }
        return cluster
      }
