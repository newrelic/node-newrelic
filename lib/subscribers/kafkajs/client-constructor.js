...
const { getClusterIdFromCache, fetchAndCacheClusterId } = require('./utils/cluster-id-cache.js')
...
module.exports = class ConstructorSubscriber extends Subscriber {
  #clusterIdCache = new Map()
  #clusterIdInFlight = new Map()
   ...
   */
    ...
    fetchAndCacheClusterId(self.#clusterIdCache, self.#clusterIdInFlight, client, client[kafkaCtx].brokers).then((id) => {
      if (id) client[kafkaCtx].clusterId = id
    }).catch(() => {})
  ...
  #wrapProducerMethod(instance, methodName) {
    ...
    return function nrWrappedMethod(...args) {
      ...
      if (ctx.transaction == null || ctx.transaction.isActive() === false) {
        ...
        const _cid0 = getClusterIdFromCache(self.#clusterIdCache, instance[kafkaCtx]?.brokers)
        if (!_cid0 && instance[kafkaCtx]?._kafkaClient) {
          fetchAndCacheClusterId(self.#clusterIdCache, self.#clusterIdInFlight, instance[kafkaCtx]._kafkaClient, instance[kafkaCtx].brokers)
            .then((id) => { if (id && instance[kafkaCtx]) instance[kafkaCtx].clusterId = id })
            .catch(() => {})
        }
        const _cidNoTx = _cid0 ?? instance[kafkaCtx]?.clusterId
        if (_cidNoTx) {
          const [data] = args
          recordClusterProduceMetrics(self.agent.metrics, _cidNoTx, batch, data)
        }
        return orig.apply(instance, args)
      }
      ...
      const _cid1 = getClusterIdFromCache(self.#clusterIdCache, instance[kafkaCtx]?.brokers)
      if (!_cid1 && instance[kafkaCtx]?._kafkaClient) {
        fetchAndCacheClusterId(self.#clusterIdCache, self.#clusterIdInFlight, instance[kafkaCtx]._kafkaClient, instance[kafkaCtx].brokers)
          .then((id) => { if (id && instance[kafkaCtx]) instance[kafkaCtx].clusterId = id })
          .catch(() => {})
      }
      const _cidInTx = _cid1 ?? instance[kafkaCtx]?.clusterId
      if (_cidInTx) {
        recordClusterProduceMetrics(self.agent.metrics, _cidInTx, batch, data)
      }
  ...
  #wrapRunForBatches(instance) {
    ...
    instance.run = function nrWrappedRunForBatches(...args) {
      ...
      args[0].eachBatch = function nrWrappedEachBatch() {
        ...
        // Emit one cluster Consume metric per message in the batch.
        // Enforce TTL: re-fetch when expired, use stale kafkaCtx.clusterId during re-fetch.
        let clusterId = getClusterIdFromCache(self.#clusterIdCache, instance[kafkaCtx]?.brokers)
        if (clusterId === undefined) {
          if (instance[kafkaCtx]?._kafkaClient) {
            fetchAndCacheClusterId(self.#clusterIdCache, self.#clusterIdInFlight, instance[kafkaCtx]._kafkaClient, instance[kafkaCtx].brokers)
              .then((id) => { if (id && instance[kafkaCtx]) instance[kafkaCtx].clusterId = id })
              .catch(() => {})
          }
          clusterId = instance[kafkaCtx]?.clusterId
        }
        if (clusterId) {
          self.agent.metrics
            .getOrCreateMetric(
              `MessageBroker/Kafka/Cluster/${clusterId}/Topic/${batch.topic}/Consume`
            )
