...
const { getClusterIdFromCache } = require('./cluster-id-cache.js')
 ...
 */
module.exports = function recordDataMetrics({ data, kafkaCtx, tx, agentMetrics }) {
  if (!tx) {
    return
  }
  ...
  // Use agentMetrics (global) so cluster metrics are not scoped to a single transaction.
