...
function fetchAndCacheClusterId(cache, inFlight, client, brokers) {
  ...
  const promise = (async () => {
    try {
      ...
      const pendingAdmin = admin
      admin = null
      // If connect() is still in flight, disconnect once it resolves to prevent a leaked socket.
      if (connectPromise) {
        connectPromise
          .then(() => { if (pendingAdmin) pendingAdmin.disconnect().catch(() => {}) })
          .catch(() => {})
      }
      try { if (pendingAdmin) await pendingAdmin.disconnect() } catch { /* ignore */ }
      return null
    } finally {
