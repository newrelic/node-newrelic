...
class Subscriber {
   ...
   * @param {string} [key] key of the callback if the callback is a field on the argument of the specified position
  ...
  traceCallback(position, key, context) {
    ...
    let callback = ArrayPrototypeAt(context.arguments, position)
    if (key) {
