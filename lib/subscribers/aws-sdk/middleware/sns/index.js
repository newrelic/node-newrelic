...
function snsMiddleware(subscriber, config, next, context) {
  ...
  return function nrSnsMiddleware(...args) {
    ...
    // In a typical producer type instrumentation we'd invoke
    // `subscriber.insertDTHeaders` here. But since SNS is not really a message
    // storage destination, headers cannot be propagated. Therefore, we do not
    // need to attach them here.
