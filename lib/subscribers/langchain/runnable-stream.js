...
class LangchainRunnableStreamSubscriber extends LangchainSubscriber {
  ...
  /**
   * Wraps the async iterator on the IterableReadableStream. It will also record the Llm
          ...
          next: self.agent.tracer.bindFunction(async function(...args) {
            try {
              ...
              if (result?.done) {
                ...
                if (typeof result?.value?.agent?.messages?.[0]?.content === 'string') {
                  // Via LangGraph
                  // TODO: assumes one message in `messages`, is this always true?
                  content += result?.value?.agent?.messages?.[0]?.content
