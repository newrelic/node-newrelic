...
class OpenAIChatCompletions extends AiMonitoringChatSubscriber {
  constructor({ agent, logger, channelName = 'nr_completionsCreate' }) {
    super({ agent, logger, channelName, packageName: 'openai' })
    this.trackingPrefix = AI.OPENAI.TRACKING_PREFIX
    this.name = AI.OPENAI.COMPLETION
  ...
  handler(data, ctx) {
    ...
    if (request.stream) {
      ...
      if (!this.streamingEnabled) {
        this.logger.debug('`ai_monitoring.streaming.enabled` is set to false, not creating segment.')
        return ctx
      }
    }
