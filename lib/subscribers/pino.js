...
const logger = require('../logger').child({ component: 'pino-subscriber' })

class PinoSubscriber extends ApplicationLogsSubscriber {
  constructor(agent) {
    super(agent, 'pino:nr_asJson')
    this.events = ['end']
  }
  ...
  handler(data, ctx) {
    ...
    return ctx.setExtras({ useMergeObj })
  }
  ...
  decorateLogLine(data) {
    ...
    if (meta) {
      if (useMergeObj === true) {
        data.arguments[0].msg += this._agent.getNRLinkingMetadata()
      } else {
        data.arguments[1] += this._agent.getNRLinkingMetadata()
      }
    }
    return useMergeObj
  }
  ...
  reformatLogLine(data, ctx) {
    ...
    const level = self?.levels?.labels?.[args[2]]
    const metadata = this._agent.getLinkingMetadata(true)
  ...
  instrumentations: [
    {
      ...
      module: { name: 'pino', versionRange: '>=7.0.0', filePath: 'lib/tools.js' },
      functionQuery: {
        functionName: 'asJson',
        kind: 'Sync'
      }
