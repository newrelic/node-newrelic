 ...
 * @property {boolean} internal When `true`, no segment is created for a call
 * whose parent segment came from this same package. Set it on any subscriber
 * whose instrumented methods delegate to one another (e.g. `child_process.exec`
 * calls `execFile`), so one logical operation yields one segment.
...
class BaseCoreSubscriber {
   ...
   */
  constructor({ agent, logger, packageName, prefix, instrumentedMethods = [] }) {
    this.agent = agent
  ...
  /**
   * Determines whether a segment should be created under `parent`. A segment is
   * skipped when this subscriber is `internal` and the parent segment came from
   * this same package -- the outer call already represents the operation.
   *
   * Skipping keeps the async context pointing at the outer segment, which is
   * what `end` and `asyncEnd` touch. A delegating subscriber that is not
   * `internal` would create a nested segment, and the outer segment's timer
   * would then never be stopped when the operation actually completes.
   *
   * @param {TraceSegment} [parent] The prospective parent segment.
   * @returns {boolean} `true` when a segment should be created.
   */
  shouldCreateSegment(parent) {
    return !(this.internal && this.packageName === parent?.shimId)
  }
  ...
  createSegment({ name, recorder, ctx }) {
    ...
    if (this.shouldCreateSegment(parent) === false) {
      this.logger.trace('Skipping segment creation for %s, parent %s is from the same package (%s) and this subscriber is internal', name, parent?.name, this.packageName)
      return ctx
    }
  ...
  /**
   * Resolves the object that {@link #instrument} will patch. Defaults to
   * requiring `packageName`. Subscribers whose target is not a requirable
   * module, e.g. the `globals` subscriber, which patches `process`, must
   * override this.
   *
   * @returns {object} The object to instrument.
   */
  resolveModule() {
    return require(this.packageName)
  }
  ...
  enable() {
    try {
      this.instrument(this.resolveModule())
    } catch (error) {
