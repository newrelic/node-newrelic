...
class BaseCoreSubscriber {
   ...
   *
   * We cannot rely on the context manager to obtain the segment here. The
   * callback runs in whichever context was active when the underlying async
   * resource was created, and that is not necessarily the context this call
   * created -- an instrumented method that delegates to another instrumented
   * method will see the inner one's segment. The segment stashed on the event
   * data in `enable` is always the one belonging to this call.
   *
