...
class ChildProcessSubscriber extends BaseCoreSubscriber {
  constructor({ agent, logger }) {
    super({ agent, logger, packageName: 'child_process', instrumentedMethods: INSTRUMENTED_METHODS })
    // `exec` delegates to `execFile`, so without this one call would produce
    // two segments for the same operation.
    this.internal = true
  }
