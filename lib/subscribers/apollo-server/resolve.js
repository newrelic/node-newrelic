...
module.exports = class ApolloResolveSubscriber extends BaseSubscriber {
  ...
  wrapResolve(origResolve, thisArg, args) {
    ...
    const [source, resolverArgs, contextValue, info] = args

    // When a resolver filter callback is registered, it is the sole authority on
    // whether to create a segment for this field. Otherwise, fall back to the
    // built-in short-circuit: when scalar capture is disabled (the default), a
    // non-top-level scalar field creates no segment, so we can skip flattening
    // the field path and the resolver args entirely -- work that the discarded
    // path used to do on every scalar field.
    const filterCallback = this.agent.customCallbacks.apollo.resolverFilterCallback
    const skipSegment =
      typeof filterCallback === 'function'
        ? !this.invokeResolverFilterCallback(filterCallback, { source, args: resolverArgs, contextValue, info })
  ...
  /**
   * Invokes the user-registered resolver filter callback to determine if a resolver segment
   * should be created for this field. Defensively catches errors thrown by the callback so
   * that a bug in user code cannot break the actual resolver call; on error, defaults to
   * creating the segment (fail safe).
   *
   * @param {Function} filterCallback the user-registered callback
   * @param {object} args the `{ source, args, contextValue, info }` passed to the callback
   * @returns {boolean} true if the resolver segment should be created
   */
  invokeResolverFilterCallback(filterCallback, args) {
    try {
      return Boolean(filterCallback(args))
    } catch (err) {
      this.logger.trace('Apollo resolver filter callback threw, defaulting to creating segment: %s', err)
      return true
    }
  }
