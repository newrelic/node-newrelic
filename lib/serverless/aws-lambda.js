...
class AwsLambda {
  ...
  patchLambdaHandler(handler) {
    ...
    // TODO: add supportability metric for streaming?
    ...
    const nrHandler = shim.bindCreateTransaction(wrappedHandler, new specs.TransactionSpec({ type: shim.BG }))
    if (isStreamHandler) {
      for (const symbol of Object.getOwnPropertySymbols(handler)) {
        logger.trace(`Setting symbol ${symbol} on handler`)
        nrHandler[symbol] = handler[symbol]
      }
      logger.trace('check streaming symbol on handler', nrHandler[HANDLER_STREAMING])
      logger.trace('check highWaterMark symbol on handler', nrHandler[HANDLER_HIGHWATERMARK])
    }
    return nrHandler
    ...
    function wrappedHandler() {
      ...
      const event = args[0]
      logger.trace('Is this a response streaming Lambda function?', isStreamHandler)
      const context = isStreamHandler === true ? args[2] : args[1]
      ...
      if (isStreamHandler === true) {
        // We don't have a callback in this case. Instead, we are wrapping up
        // the response stream.
