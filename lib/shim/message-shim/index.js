...
const Transport = require('../../transaction/distributed-trace/transport.js')
...
function MessageShim(agent, moduleName, resolvedName, shimName, pkgVersion) {
  ...
  this._metrics = null
  this._transportType = Transport.UNKNOWN
}
...
for (const [libName, libType] of Object.entries(LIBRARY_NAMES)) {
  Shim.defineProperty(MessageShim, libName, libType)
  Shim.defineProperty(MessageShim.prototype, libName, libType)
}

// Add constants to the shim for message broker destination types.
MessageShim.DESTINATION_TYPES = DESTINATION_TYPES
for (const [destName, destType] of Object.entries(DESTINATION_TYPES)) {
  Shim.defineProperty(MessageShim, destName, destType)
  Shim.defineProperty(MessageShim.prototype, destName, destType)
}
...
function recordProduce(nodule, properties, recordNamer) {
  ...
  return this.record(nodule, properties, function recordProd(shim) {
    ...
    msgDesc.inContext = function generateDTHeaders() {
      if (msgDesc.headers) {
        shim.insertDTRequestHeaders(msgDesc.headers)
      }
      ...
      if (msgDesc.messageHeaders) {
        ...
        msgDesc.messageHeaders((headers) => {
          shim.insertDTRequestHeaders(headers)
        })
      }
    }
