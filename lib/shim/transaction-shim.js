 ...
 *
 * Each transaction type value is exposed directly on the `TransactionShim`
 * class (and its prototype) as a static member, e.g. `TransactionShim.WEB`.
 *
 * @readonly
 * @memberof TransactionShim
 * @enum {string}
 */
for (const [name, type] of Transaction.TYPES.entries) {
 ...
 * @enum {string}
 */
Shim.defineProperty(TransactionShim, 'TRANSPORT_TYPES', () => Transport)
Shim.defineProperty(TransactionShim.prototype, 'TRANSPORT_TYPES', () => Transport)
...
function bindCreateTransaction(nodule, property, spec) {
  ...
  if (!Transaction.TYPES.isValid(spec.type)) {
    this.logger.error(
