...
// TODO: Remove try-catch when deprecating Node <4.
var es6 = null
try {
  es6 = require('./es6')
} catch (err) {
...
/**
 * Enumeration of segment parameters which ignore the `attributes.enabled`
 * configuration value.
 * @private
 *
 * @readonly
...
function record(nodule, properties, recordNamer) {
  ...
  return this.wrap(nodule, properties, function makeWrapper(shim, fn, name) {
    ...
    return function wrapper() {
      ...
      var parent = segDesc.parent || shim.getActiveSegment()
      if (!parent) {
...
function copySegmentParameters(segment, parameters) {
  var ignoredParams = this.agent.config.attributes.exclude
  var captureParams = this.agent.config.attributes.enabled
