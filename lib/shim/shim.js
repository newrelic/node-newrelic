...
function bindSegment(nodule, property, segment, full) {
  ...
  // function.
  // TODO: Remove this once Node v4 is deprecated.
...
function _wrap(shim, original, name, spec, args) {
  ...
  if (wrapped && wrapped !== original) {
    if (shim.isFunction(wrapped)) {
      wrapped = _fixArity(shim, original, wrapped)
...
function _bindPromise(shim, promise, segment) {
  // If this promise has a "finally" hook use that, it is guaranteed to be
  // without sideeffect.
  if (promise.finally instanceof Function) {
    return promise.finally(thenTouch)
  }

  promise.then(thenTouch, thenTouch)
  return promise.catch(function catchRethrow(err) {
    throw err // NOTE: This is not our error, don't blame us!
  })
...
// Starting in what we believe to be Node v5 you can set the name and length of
// a function as properties. This is more ideal than wrapping a function.
// TODO: Remove this once Node v4 has been deprecated.
var _fixArity = semver.satisfies(process.version, '>=5')
  ? _fixArity_prop : _fixArity_newFunc

function _fixArity_prop(shim, original, wrapper) {
...
function _fixArity_newFunc(shim, original, wrapper) {
  ...
  var renamed = new Function('argsToArray, wrapper', [
    'var wrapperFunction = function ' + name + '(' + args + ') {',
    '  if (this instanceof wrapperFunction) {',
    '    var fnArgs = argsToArray.apply(null, arguments)',
  ...
  shim.setInternalProperty(renamed, '__NR_name', wrapper.name)
  return renamed
}
