...
function record(nodule, properties, recordNamer) {
  ...
  return this.wrap(nodule, properties, function makeWrapper(shim, fn, name) {
    ...
    function _applyRecorderSegment(segment, ctx, args, segDesc) {
      ...
      var ret
      try {
        ret = shim.applySegment(fn, segment, true, ctx, args, segDesc.beforeCall)
 ...
 * execution.
 *
 * - `applySegment(func, segment, full, context, args[, beforeCallCB])`
 *
 * @memberof Shim.prototype
 *
 * @param {Function} func
...
function applySegment(func, segment, full, context, args, beforeCallCB) {
  ...
  if (beforeCallCB && typeof beforeCallCB === 'function') {
    beforeCallCB()
  }
