  ...
  return this.wrap(nodule, properties, function returnWrapper(shim, fn, fnName) {
    ...
    const handler = {
      get: function getTrap(target, prop) {
        ...
        // TODO: maybe Reflect.get(target, prop)
        return target[prop]
      },
      ...
      apply: function applyTrap(target, thisArg, proxyArgs) {
        // Call the underlying function. If this was called as a constructor, call
        // the wrapped function as a constructor too.
        // TODO: maybe Reflect.apply(thisArg, proxyArgs)
    ...
    if (shim.agent.config.transaction_tracer.hide_internals) {
      handler.defineProperty = function definePropertyTrap(target, key, descriptor) {
        if (key === '__NR_unwrap') {
      ...
      handler.set = function setTrap(target, key, val) {
        if (key === '__NR_unwrap') {
