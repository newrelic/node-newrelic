...
function _bindStream(shim, stream, segment, spec) {
  ...
  if (shim.isFunction(stream.on)) {
    ...
    if (event) {
      ...
      var segmentName = ' calls to event: ' + spec.event
      stream.on(spec.event, function rowEventHandler() {
        if (++emitCount === 1) {
          segment && segment.touch()
          if (spec.shouldCreateSegment) {
            eventSegment = shim.createSegment(emitCount + segmentName, segment)
          } else {
        ...
        if (eventSegment) {
          if (eventSegment !== segment) {
            eventSegment.name = emitCount + segmentName
          }
          eventSegment.touch()
        }
      })
