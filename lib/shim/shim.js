...
var logger = require('../logger.js').child({shim: 'Shim'})
...
function Shim(agent) {
  if (!agent) {
    throw new Error('Shim must be initialized with an agent.')
  }
  this._agent = agent
}
module.exports = Shim
...
defineProperties(Shim.prototype, {
  /**
   * Constants for referencing elements of an argument array.
   *
   ...
   *
   * @readonly
   * @member {number} Shim.prototype.FIRST
   * @member {number} Shim.prototype.SECOND
   * @member {number} Shim.prototype.THIRD
   * @member {number} Shim.prototype.FOURTH
   * @member {number} Shim.prototype.LAST
   */
  FIRST: 0,
  SECOND: 1,
  THIRD: 2,
...
function bindSegment(nodule, property, segment, full) {
  ...
  return this.wrap(nodule, property, function wrapFunc(shim, func, name) {
    if (!shim.isFunction(func)) {
      return func
    }

    // Wrap up the function with this segment.
    return _makeBindWrapper(shim, func, segment || shim.getSegment(), full || false)
  })
}
...
function bindCallbackSegment(args, cbIdx, parentSegment) {
  ...
  if (this.isFunction(cb)) {
    ...
    var wrapper = this.bindSegment(function callbackWrapper() {
      var segment = shim.createSegment(segmentName, parentSegment)
      if (segment) {
        segment.async = false
      }
