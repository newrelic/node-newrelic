...
function createSegment(name, recorder, parent) {
  ...
  if (segment) {
    ...
    if (hasOwnProperty(opts, 'parameters')) {
      this.copySegmentParameters(segment, opts.parameters)
    }
    this.logger.trace({segment: opts}, 'Created segment')
  } else {
    this.logger.debug({segment: opts},'Failed to create segment')
  }
...
function setInternalProperty(obj, name, val) {
  ...
  if (!(obj instanceof Object)) {
    this.logger.debug('obj is not an Object; not setting value')
    return
  }
...
function _wrap(shim, original, name, spec, args) {
  ...
  if (wrapped && wrapped !== original) {
    if (shim.isFunction(wrapped) && spec.matchArity) {
