...
function _recordMiddleware(shim, middleware, spec) {
  ...
  function middlewareWithCallbackRecorder(shim, fn, fnName, args) {
    ...
    if (shim.isFunction(spec.next)) {
      nextWrapper = _makeNextBinder(route, txInfo, spec.next)
    } else {
  ...
  function middlewareWithPromiseRecorder(shim, fn, fnName, args) {
    ...
    // TODO: future instrumentations will likely need more granular stack manipulation
    // txInfo.segmentStack.push()
...
function _makeNextBinder(route, txInfo, wrapNext) {
