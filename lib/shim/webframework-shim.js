...
WebFrameworkShim.prototype.setTransactionName = setTransactionName
 ...
 *
 * @param {bool} [isEndpoint=false]
 *  True if this middleware is believed to be the endpoint middleware for this
 *  route.
 *
...
function setDispatcher() {
  // TODO: docs above say this uses the name set with setFramework, but
  // that's not the case - it uses moduleName
  this.agent.environment.setFramework(this.moduleName)
  this.agent.environment.setDispatcher(this.moduleName)
}

/**
 * Sets the name to be used for the transaction currenty in scope.
 *
...
function wrapMiddlewareMounter(nodule, properties, spec) {
  ...
  return this.wrap(nodule, properties, function wrapMounter(shim, fn, fnName) {
    ...
    return function wrappedMounter() {
      ...
      _wrapMiddlewares.call(this, routeIdx, endpointIdx, args)
...
function _recordMiddleware(shim, middleware, type, route, spec) {
  ...
  if (SPECIAL_MIDDLEWARE.hasOwnProperty(type)) {
    metricName = shim._metrics.PREFIX + SPECIAL_MIDDLEWARE[type] + ': '
    if (type === MIDDLEWARE_TYPE_NAMES.ENDPOINT) {
      metricName += name
    }
  } else {
    metricName = shim._metrics.MIDDLEWARE + name + '/'
  }
  var segmentName = metricName + (route || '/')
  ...
  return shim.record(middleware, function middlewareRecorder(shim, fn, fnName, args) {
    ...
    if (shim.isFunction(spec.next)) {
      ...
      nextIdx = shim.normalizeIndex(args.length, spec.next)
      if (nextIdx !== null && args[nextIdx] instanceof Function) {
        nextWrapper = _makeNextBinder(txInfo, function wrapNext(shim, fn, name, args,
            wrap) {
          wrap(args, nextIdx)
        })
      }
