...
function bindRowCallbackSegment(args, cbIdx, parentSegment) {
  ...
  var callCounter = 0
  var segment = null
  var segmentName = ' calls to row callback: ' + this.getName(cb)
  var shim = this
  ...
  var wrapper = this.bindSegment(function rowCallbackWrapper() {
    ...
    if (++callCounter === 1) {
      ...
      segment = shim.createSegment(callCounter + segmentName, realParent)
    ...
    if (segment) {
      segment.name = callCounter + segmentName
    }
