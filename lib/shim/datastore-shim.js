...
metrics.datastores = {
  DB: metrics.DB,
  CASSANDRA: metrics.CASSANDRA,
  REDIS: metrics.REDIS
}

var DATASTORE_METRICS = Object.keys(metrics.datastores)
 ...
 */
function DatastoreShim(agent, datastoreId) {
...
DATASTORE_METRICS.forEach(function defineDatastoreMetricEnum(dsName, i) {
  Shim.defineProperty(DatastoreShim, dsName, i)
  Shim.defineProperty(DatastoreShim.prototype, dsName, i)
})

Shim.defineProperty(DatastoreShim.prototype, 'logger', logger)
 ...
 *  The ID of datastore the agent already knows about, or the name of datastore.
 */
function setDatastore(datastoreId) {
  if (this.isNumber(datastoreId)) {
    this._datastoreId = datastoreId
    this._metrics = metrics.datastores[DATASTORE_METRICS[datastoreId]]
  } else {
    this._metrics = {
      PREFIX: datastoreId,
      STATEMENT: metrics.DB.STATEMENT + '/' + datastoreId + '/',
      OPERATION: metrics.DB.OPERATION + '/' + datastoreId + '/',
      INSTANCE: metrics.DB.INSTANCE + '/' + datastoreId + '/'
    }
  }
