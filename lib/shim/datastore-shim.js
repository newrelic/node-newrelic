...
function bindRowCallbackSegment(args, cbIdx, parentSegment) {
  ...
  this.wrap(args, idx, function wrapRowCallback(shim, fn, fnName) {
    ...
    return function rowCallbackWrapper() {
      ...
      if (firstCall) {
        ...
        segment = shim.createSegment('Row Callback: ' + fnName, realParent)
      ...
      // Run the actual callback under our child segment.
      return shim.applySegment(fn, segment, true, this, arguments)
    }
  })
}
