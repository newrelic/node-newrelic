...
MessageShim.prototype.recordProducer = recordProducer
MessageShim.prototype.recordConsumer = recordConsumer
 ...
 *  consumed.
 *
 * @see MessageShim#recordProducer
 * @see MessageShim#recordConsumer
 */
 ...
 *
 * - `recordProducer(nodule, properties, recordNamer)`
 * - `recordProducer(func, recordNamer)`
 *
 ...
 * @see MessageFunction
 */
function recordProducer(nodule, properties, recordNamer) {
  ...
  return this.record(nodule, properties, function recordProduce(shim) {
 ...
 *
 * - `recordConsumer(nodule, properties, recordNamer)`
 * - `recordConsumer(func, recordNamer)`
 *
 * The resulting wrapped methods will record their executions using the messaging
 ...
 * @see MessageFunction
 */
function recordConsumer(nodule, properties, recordNamer) {
  if (this.isFunction(properties)) {
    // recordConsumer(func, recordNamer)
    recordNamer = properties
    properties = null
 ...
 * the message library. See {@link MessageShim#recordConsumer} for recording
 * the consumer itself.
 *
...
function _makeConsumeSegment(shim, msgDesc, checkCAT) {
  ...
  var segDesc = {
    ...
    callback: msgDesc.callback || null,
    recorder: genericRecorder,
    parameters: msgDesc.parameters || {}
  }

  if (msgDesc.routingKey && !segDesc.parameters.routing_key) {
    segDesc.parameters.routing_key = msgDesc.routingKey
  }

  if (!shim.agent.config.message_tracer.segment_parameters.enabled) {
    delete segDesc.parameters
  }
