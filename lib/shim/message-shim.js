...
MessageShim.prototype.recordSubcribedConsume = recordSubcribedConsume
 ...
 */
function recordSubcribedConsume(nodule, properties, spec) {
  if (!nodule) {
    this.logger.debug('Not wrapping non-existent nodule.')
    return nodule
  }
  ...
  var wrapped = this.wrap(nodule, properties, function wrapSubscribe(shim, fn) {
    ...
    return function wrappedSubscribe() {
      ...
      if (consumerIdx !== null) {
        args[consumerIdx] = shim.wrap(args[consumerIdx], _wrapConsumer)
      }
  ...
  function _wrapConsumer(shim, consumer, cName) {
    ...
    return shim.bindCreateTransaction(function createConsumeTrans() {
      ...
      if (msgDesc.headers) {
        shim.handleCATHeaders(msgDesc.headers, tx.baseSegment)
      }
