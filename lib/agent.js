...
const createSpanAggregator = require('./spans/create-span-aggregator')
const {
...
function Agent(config) {
  ...
  this.spanAggregator = createSpanAggregator(config, this)
...
Agent.prototype.startStreaming = function startStreaming() {
  if (this.spanAggregator.isStream && this.spanAggregator.enabled) {
    this.spanAggregator.start()
  }
}
...
Agent.prototype._beforeMetricDataSend = function _beforeMetricDataSend() {
  ...
  if (this.spanAggregator.isStream) {
    this.spanAggregator.createMetrics()
  }
}
