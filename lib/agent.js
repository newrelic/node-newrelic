...
const validateLicenseKey = require('./validate-license-key')
...
Agent.prototype.start = function start(callback) {
  ...
  } else if (validateLicenseKey(this.config.license_key) === false) {
    logger.error('An invalid license key was supplied.')
    this.healthReporter.setStatus(HealthReporter.STATUS_INVALID_LICENSE_KEY)

    this.setState('errored')
    sampler.stop()
    return process.nextTick(function onNextTick() {
      agent.healthReporter.stop(() => {
        callback(new Error('Not starting with invalid license key!'))
      })
    })
  }
