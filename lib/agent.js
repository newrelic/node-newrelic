...
Agent.prototype.start = function start(callback) {
  ...
  // Build the profiling source mapper once, so it can be reused across profiling
  // start/stop cycles; awaited in `onConnect` before harvests start the profilers.
...
Agent.prototype.onConnect = function onConnect(shouldImmediatelyHarvest, callback) {
  ...
  // Harvests start the profilers, which consume the source mapper.
  // Wait for its build before scheduling, so the first profiling
  // session resolves frames to their original source.
  Promise.resolve(this.sourceMapperReady).then(() => {
