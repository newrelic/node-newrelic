...
class Agent extends EventEmitter {
  ...
  start(callback) {
    ...
    if (this.config.agent_enabled !== true) {
      logger.warn(
        'The New Relic Node.js agent is disabled by its configuration. ' + 'Not starting!'
      )
   ...
   * @param callback
   */
  onConnect(shouldImmediatelyHarvest, callback) {
  ...
  _reconfigureAggregators(config) {
  ...
  _scheduleHarvests(shouldImmediatelyHarvest, callback) {
   ...
   */
  _transactionFinished(transaction) {
  ...
  setLambdaFunctionVersion(functionVersion) {
    if (this.collector instanceof ServerlessCollector) {
  ...
  _addIntrinsicAttrsFromTransaction(transaction) {
