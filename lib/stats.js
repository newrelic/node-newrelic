...
function MetricDataSet(unscopedStats, scopedStats, metricIds) {
  ...
  this.toJSON = function () {
    ...
    for (var scope in scopedStats) {
      if (scopedStats.hasOwnProperty(scope)) {
...
function StatsEngine(logger) {
  ...
  this.harvest = function (nrService) {
    ...
    nrService.sendMetricData(lastSendTime / 1000, Date.now() / 1000, md);
  };
  ...
  this.getMetricData = function () {
    var md = new MetricDataSet(unscopedStats, scopedStats, this.metricIds);
    this.clear();
    return md;
  };
