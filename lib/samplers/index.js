...
class Samplers {
  ...
  #sendCoreTracingMetrics(agent) {
    ...
    if (this.partialEnabled) {
      ...
      metrics.getOrCreateMetric(`${SAMPLERS.PARTIAL.SAMPLED}/${this.#determineSamplerType(this.partialRemoteParentSampled)}`)
      metrics.getOrCreateMetric(`${SAMPLERS.PARTIAL.NOT_SAMPLED}/${this.#determineSamplerType(this.partialRemoteParentNotSampled)}`)
    }
    if (this.fullEnabled) {
      // Supportability/Nodejs/PartialGranularity/<sampler section>/<sampler type>
      metrics.getOrCreateMetric(`${SAMPLERS.FULL.ROOT}/${this.#determineSamplerType(this.root)}`)
      metrics.getOrCreateMetric(`${SAMPLERS.FULL.SAMPLED}/${this.#determineSamplerType(this.remoteParentSampled)}`)
      metrics.getOrCreateMetric(`${SAMPLERS.FULL.NOT_SAMPLED}/${this.#determineSamplerType(this.remoteParentNotSampled)}`)
    }
  }
