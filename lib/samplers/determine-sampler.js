...
function determineSampler({ agent, config }) {
  ...
  // TODO: What about dt?.sampler?.remote_parent_sampled?.trace_id_ratio_based
  //  and dt?.sampler?.remote_not_sampled?.trace_id_ratio_based?
  if (dt?.sampler?.root?.trace_id_ratio_based?.ratio) {
    return new TraceIdRatioBasedSampler({
      agent,
      ratio: dt.sampler.root.trace_id_ratio_based.ratio
    })
  }
  if (dt?.sampler?.root?.trace_id_ratio_based && !dt?.sampler?.root?.trace_id_ratio_based?.ratio) {
    ...
    agent.logger.warn('trace_id_ratio_based sampler is configured without a ratio, defaulting to adaptive sampler')
  }
