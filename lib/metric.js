exports.externalMetrics = function (host, library, operation) {
  return function (tracer, unscopedStats, scopedStats) {
    ...
    unscopedStats.byName("External/all" + (tracer.getTransaction().isWebTransaction() ? "Web" : "Other")).
...
exports.recordWebTransactionMetrics = function (metricNormalizer, unscopedStats, requestUri, durationInMillis, totalExclusiveInMillis, responseStatusCode) {
  ...
  unscopedStats.byName(frontendMetricName).recordValueInMillis(durationInMillis, Math.max(0, durationInMillis - totalExclusiveInMillis));
...
function MetricNormalizer(logger) {
  ...
  this.parseMetricRules = function (connectResponse) {
    ...
    self.rules = connectResponse.url_rules;
    if (self.rules) logger.debug("Received " + self.rules.length + " metric naming rule(s)");
  };
