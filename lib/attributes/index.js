...
const byteUtils = require('../util/byte-limit.js')
const properties = require('../util/properties.js')
const { DESTINATIONS } = require('../config/attribute-filter.js')
...
class Attributes {
   ...
   * attribute values.
   * @param {AgentLogger} [params.logger] Logger instance to use.
   */
  constructor({
    scope,
    limit = Infinity,
  ...
  addAttribute(destinations, key, value, truncateExempt = false) {
    ...
    if (!this.isValidLength(key)) {
      return this.#logger.warn('Length limit exceeded for attribute name, not adding: %s', key)
    }
