...
function _postLoad(agent, nodule, name, resolvedName) {
  const instrumentationKey = isESMImport(name, resolvedName)
    ? getModuleNameFromFilepath(name)
    : shimmer.getInstrumentationNameFromModuleName(name)

  const registeredInstrumentation = shimmer.registeredInstrumentations[instrumentationKey]
  ...
  if (hasPostLoadInstrumentation) {
    ...
    return instrumentPostLoad(agent, nodule, instrumentationKey, resolvedName)
  }
...
function _onResolveFileName(agent, requiredNameOrPath, resolvedFilepath) {
  const instrumentationKey = isESMImport(requiredNameOrPath, resolvedFilepath)
    ? getModuleNameFromFilepath(requiredNameOrPath)
    : shimmer.getInstrumentationNameFromModuleName(requiredNameOrPath)
  const registeredInstrumentation = shimmer.registeredInstrumentations[instrumentationKey]
  ...
  if (hasResolvedFileInstrumentation) {
    ...
    _instrumentOnResolved(agent, instrumentationKey, resolvedFilepath)
  }
}
...
/**
 * Function for determining if a given moduleName is from ESM
 * A call is from ESM if the moduleName is absolute and fully resolved (with file extension)
 *
 * We have to do the last check because of the usage of `shim.require()`, which will end up calling
 * `require()` with an absolute file path, which makes it look like an ESM generated filepath
 *
 * @param {string} moduleName the module name from import/require
 * @param {string} filepath the fully resolved filepath from Module
 * @returns {boolean} whether or not the given inputs are from ESM land
 */
function isESMImport(moduleName, filepath) {
  return path.isAbsolute(moduleName) && moduleName === filepath
}

/**
 * Function for taking a fully resolved filepath and distilling that into a package name
 * Note that we have to do the package.json check because we use shim.require() to get the package.json
 * of various module to determine versioning info, which flows through this same instrumentation of Module
 *
 * @param {string} filepath - fully resolved filepath of package/module
 * @returns {string} the "shortened" package name if it can be determined, otherwise the original input
 */
function getModuleNameFromFilepath(filepath) {
  const pkgInfo = parse(filepath)
