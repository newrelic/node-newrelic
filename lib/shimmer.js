...
var shimmer = module.exports = {
  ...
  bootstrapInstrumentation: function bootstrapInstrumentation(agent) {
    ...
    // inject async hooks if available
    var injectHookFlag = agent.config.feature_flag.await_support
    if (semver.satisfies(process.version, '>=8') && injectHookFlag) {
      CORE_INSTRUMENTATION.async_hooks = 'async_hooks.js'
    }
