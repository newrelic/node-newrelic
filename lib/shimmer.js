  ...
  patchModule: function patchModule(agent) {
    ...
    shimmer.wrapMethod(Module, 'Module', '_resolveFilename', function wrapRes(resolve) {
      return function wrappedResolveFilename() {
        ...
        filepathStack.push(resolve.apply(this, arguments))
        return filepathStack[filepathStack.length - 1]
      }
    })

    shimmer.wrapMethod(Module, 'Module', '_load', function wrapLoad(load) {
      return function wrappedLoad(file) {
        // Since the load call will resolve and load more paths, we must wait till the
        // subtree of modules below this has finished loading before we consider this
        // file finished.  This mirrors the post-order behavior of module loading.
        const m = load.apply(this, arguments)
        const res = _postLoad(agent, m, file, filepathStack.pop())
        return res
      }
    })
  },
