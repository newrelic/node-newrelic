...
var path        = require('path')
  ...
  , manager     = require(path.join(__dirname, 'transaction', 'manager'))
...
var shimmer = module.exports = {
   ...
   * overkill.
   */
  generateShim : function generateShim(next) {
    if (!next) return;

    agents.forEach(function (agent) {
      var transaction = callstack.findAnnotation(manager.NR_TRANSACTION_NAME);

      if (transaction) next[manager.NR_TRANSACTION_NAME] = transaction;
    });
...
require(path.join(__dirname, 'hook'))(shimmer.generateShim);
