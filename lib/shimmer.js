...
var shimmer = module.exports = {
  ...
  restoreMethod : function restoreMethod(nodule, method) {
    var wrapped = '__NR_ORIG_' + method;
    if (nodule[wrapped]) {
      logger.debug('restoring ' + getObjectClass(nodule) + '.' + wrapped + ' to ' + method);
      nodule[method] = nodule[wrapped];
      delete nodule[wrapped];
    }
    else {
      logger.debug('wrapped version of ' + getObjectClass(nodule) + '.' + method + ' not found!');
    }
  },
  /**
  ...
  unpatchModule : function unpatchModule() {
    ...
    shimmer.restoreMethod(module, '_load');
  },
  /**
