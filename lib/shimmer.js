  ...
  setupSubscribers: function setupSubscribers(agent) {
    ...
    for (const subscriberConfigList of Object.values(subscriptions)) {
      for (const subscriberConfig of subscriberConfigList) {
        ...
        // Forward channelName so subscribers reused across multiple channels (e.g. mongodb) can bind to the correct one.
        const channelName = subscriberConfig.instrumentations[0]?.channelName
        const subscriber = new Subscriber({ agent, logger, channelName })
