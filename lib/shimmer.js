...
const shimmer = (module.exports = {
  ...
  registerCoreInstrumentation(agent) {
    ...
    for (const [mojule, core] of Object.entries(CORE_INSTRUMENTATION)) {
      if (agent.config.instrumentation?.[mojule].enabled === false) {
        ...
        this._coreInstrumentations[mojule] = _firstPartyInstrumentation(agent, filePath, shim, uninstrumented, mojule)
      }
    }
  },
  ...
  removeHooks(agent) {
    ...
    if (this._coreInstrumentations) {
      for (const instrumentation of Object.values(this._coreInstrumentations)) {
