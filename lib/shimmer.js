...
var loadInstrumentationFile = function loadInstrumentationFile(agent, shortName, fileName, nodule) {
  ...
  try {
    ...
    logger.debug("Failed to instrument module " +
                 path.basename(shortName, ".js") + ": " + error.stack);
    return false;
  }
...
var moduleLoad = function moduleLoad(agent, nodule, name) {
  ...
  if (!existsSync) existsSync = path.existsSync;
...
var shimmer = module.exports = {
  ...
  wrapAgent : function wrapAgent(wrapped) {
    if (!(wrapped && wrapped.config)) return logger.verbose('Must be passed an agent to wrap.');

    logger.debug('wrapping agent ' + wrapped.config.app_name + ' with shim');
    agent = wrapped;
  },
