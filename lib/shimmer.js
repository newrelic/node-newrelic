...
function instrument(agent, nodule, moduleName) {
  ...
  if (instrumentation.type === MODULE_TYPE.DATASTORE) {
    shim = new shims.DatastoreShim(agent, moduleName, instrumentation.datastoreName)
  } else if (instrumentation.type === MODULE_TYPE.WEB_FRAMEWORK) {
    shim = new shims.WebFrameworkShim(agent, moduleName, instrumentation.datastoreName)
  } else {
    shim = new shims.Shim(agent, moduleName)
  }
...
function _postLoad(agent, nodule, name) {
  ...
  if (registeredInstrumentations[instrumentation]) {
    ...
    return instrument(agent, nodule, instrumentation)
  }
