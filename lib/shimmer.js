...
const shimmer = (module.exports = {
  ...
  setupSubscribers: function setupSubscribers(agent) {
    this._subscribers = {}
    for (const subscriberConfig of Object.values(subscriptions)) {
      if (Array.isArray(subscriberConfig)) {
        // Some packages have multiple subscribers, not just one.
        for (const s of subscriberConfig) {
          const subscriber = setupSubscriber(s, agent, logger)
          if (subscriber) this._subscribers[subscriber.id] = subscriber
        }
      } else {
        const subscriber = setupSubscriber(subscriberConfig, agent, logger)
        if (subscriber) this._subscribers[subscriber.id] = subscriber
      }
    }
  },
...
function setupSubscriber(subscriberConfig, agent, logger) {
