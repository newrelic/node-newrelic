...
function facts(agent, callback) {
  ...
  a.parallel({
    ...
    // TODO: remove feature gating when fully integrated
    if (agent.config.feature_flag.event_harvest_config) {
      results.event_harvest_config = {
        harvest_limits: {
          analytic_event_data: agent.config.transaction_events.max_samples_per_minute,
