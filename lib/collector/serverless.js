...
class ServerlessCollector {
   ...
   *
   * @param {string} functionVersion version indicator for Lambda function
   */
  setLambdaFunctionVersion(functionVersion) {
    this.metadata.function_version = functionVersion
  }
  ...
  flushPayload(cb) {
    ...
    zlib.gzip(toFlush, function flushCompressed(err, compressed) {
      ...
      collector.#doFlush(compressed.toString('base64'))
  ...
  flushPayloadSync() {
    ...
    if (this._agent.otel?.metrics?.flushToString) {
      // Unfortunately, the OTEL API is designed in such a way that we cannot
  ...
  #finalizeSync() {
    ...
    try {
      // Per serverless spec, this payload is always gzipped
      this.#doFlush(zlib.gzipSync(toFlush).toString('base64'), true)
    } catch (err) {
      logger.warn('Encountered an error while attempting to compress payload', err)
    } finally {
