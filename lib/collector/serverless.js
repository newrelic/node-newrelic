...
const METHOD_TO_KEY = {
  ...
  transactionSampleData: 'transaction_sample_data',
  analyticsEvents: 'analytic_event_data',
...
class ServerlessCollector {
  ...
  populateDataSync(data) {
    ...
    Object.keys(data).forEach(function setData(key) {
      collector.payload[key] = data[key]
    })
  }
  ...
  flushPayloadSync() {
    ...
    try {
      fs.writeSync(1, JSON.stringify([
        PAYLOAD_VERSION,
        PAYLOAD_MARKER,
        zlib.gzipSync(toFlush).toString('base64')
      ]) + '\n')
    } catch (err) {
      logger.warn('Encountered an error while attempting to compress payload', err)
    } finally {
