...
const { ERR_CODES, constructionError, generalError } = require('./errors')
...
function RemoteMethod(name, agent, endpoint) {
  if (!name) {
    throw constructionError(
      'Must include name of method to invoke on collector.',
      ERR_CODES.NR_REMOTE_METHOD_CONSTRUCT
    )
  }
  if (!agent) {
    throw constructionError('Must include an agent instance.', ERR_CODES.NR_REMOTE_METHOD_CONSTRUCT)
  }
...
RemoteMethod.prototype._safeRequest = function _safeRequest(options) {
  if (!options) {
    throw generalError(
      ...
      ERR_CODES.NR_REMOTE_METHOD_MISSING_REQUIRED_PARAM
    )
  }
  if (!options.host) {
    throw generalError(
  ...
  if (!isValidLength(options.body, maxPayloadSize)) {
    ...
    throw generalError(
      ...
      ERR_CODES.NR_REMOTE_METHOD_MAX_PAYLOAD_SIZE_EXCEEDED
    )
  }
