...
/**
 * A replacer to be used when serializing `log_event_data`
 * It basically just calls `toString` on a value that is a big int.
 * see: https://new-relic.atlassian.net/browse/NR-41286
 * see: https://github.com/newrelic/node-newrelic/pull/1336
 *
 * @param {string} key in object that is being serialized
 * @param {*} value in object that is being serialized
 * @returns {string} the serialized value
 */
function logEventReplacer(_key, value) {
...
RemoteMethod.prototype.serialize = function serialize(payload, callback) {
  let res
  try {
    // Avoiding defining a replacer for _every_ serialization call to invoke
    // the v8 optimizations around `JSON.stringify`
    // see: https://v8.dev/blog/json-stringify#limitations
    const replacer = this.name === 'log_event_data' ? logEventReplacer : null
    if (this.name === 'span_event_data') {
      debugger
      console.log('calling stringify on payload')
    }
    res = stringify(payload, replacer) 
  } catch (error) {
...
RemoteMethod.prototype._reportDataUsage = function reportDataUsage(sent, received) {
  ...
  const responseByteLength = received ? byteLength(stringify(received)) : 0
