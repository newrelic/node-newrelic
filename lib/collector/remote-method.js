...
RemoteMethod.prototype._safeRequest = function _safeRequest(options) {
  ...
  // We check if audit logging is enabled before attempting to log because
  // this is a hot path and there is at least one expensive operation to
  // perform before passing data to the logger: parsing the URL path.
  if (logger.auditEnabled() === true) {
    const logBody = Buffer.isBuffer(options.body)
      ? 'Buffer ' + options.body.length
      : options.body
    logger.audit(
      { body: logBody },
      'Posting to %s://%s:%s%s',
