...
function wrapReply(wrap, reply, isPreHandler) {
  // pre-handlers don't use any reply interface methods besides `response`
  if (!isPreHandler) {
    wrap(reply, 'continue', false)
    // redirect and close always mark the end of a route operation
    wrap(reply, 'close', true)
    wrap(reply, 'redirect', true)
  }
  wrap(reply, 'response', !isPreHandler)
}
