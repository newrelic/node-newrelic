      ...
      function _wrapRoute(shim, route) {
        ...
        if (shim.isArray(route)) {
          ...
          if (route.config.pre) {
            ...
            for (var i = 0; i < route.config.pre.length; ++i) {
              route.config.pre[i] = wrapPreHandler(shim, route.config.pre[i], route.path)
            }
          }
...
function wrapPreHandler(shim, container, path) {
  if (shim.isArray(container)) {
    for (var i = 0; i < container.length; ++i) {
      wrapPreHandler(shim, container[i], path)
    }
    return container
...
function buildMiddlewareSpec(shim, path, preHandler) {
  return {
    route: path,
    ...
    next: function wrapNext(shim, fn, fnName, args, wrap) {
      ...
      if (preHandler) {
        wrap(args, 1) // if this is `pre` handler, we want to wrap reply itself
      } else {
