...
module.exports = function initialize(agent, connect) {
  function wrapHandle(handle) {
    return function wrappedConnectHandle(req, res, next) {
      return agent.tracer.callbackProxy(handle).apply(this, req, res, next);
    };
  }
  ...
  /**
   * Connect 1 and 2 are very different animals.
   */
  ...
  switch (version) {
    ...
    case '2':
      shimmer.wrapMethod(connect && connect.proto, 'use', 'use', function (use) {
        var interceptor = {
          route : '',
