 ...
 * @param {string} pkgVersion The version string from Prisma's package manifest.
 * @returns {object} returns prisma datasource connection configuration { provider, url }
 */
function extractPrismaDatasource(client, pkgVersion) {
  if (semver.gte(pkgVersion, '7.0.0') === true) {
    // We know it is not cool to access `._engine`. But we cannot find an
    // equivalent means in v7+ as is done in <=v6.
    return {
      provider: client._engine.config.activeProvider,
      url: client._engine.config.adapter.config.connectionString
    }
  }
      ...
      return new QuerySpec({
        ...
        inContext: function inContext() {
          if (!client[prismaConnection]) {
            try {
              const activeDatasource = extractPrismaDatasource(client, pkgVersion)
