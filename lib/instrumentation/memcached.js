exports.initialize = function (agent, trace, memcached) {
  function recordMemcacheMetrics(tracer, unscopedStats, scopedStats) {
    [unscopedStats.getStats("Memcache/all"),
     ...
     unscopedStats.getStats(tracer.metricName),
     scopedStats.getStats(tracer.metricName)].forEach(function (stat) {
  ...
  memcached.prototype.command = function () {
    ...
    if (!tx) return origFn.apply(this,arguments);
    ...
    tracer.metricName = 'Memcache/' + (cmdObj.type||'Unknown');
