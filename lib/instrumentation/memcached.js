...
var path            = require('path')
  ...
  , recordDatastore = require(path.join(__dirname, '..', 'metrics',
  ...
  shimmer.wrapMethod(memcached && memcached.prototype,
    ...
    return tracer.segmentProxy(function () {
      ...
      var metacall = arguments[0]()
        ...
        , segment  = state.getSegment().add(name, recordDatastore)
        , keys     = wrapKeys(metacall)
        ;

      if (keys.length > 0) segment.parameters.key = JSON.stringify(keys);
