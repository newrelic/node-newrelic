...
module.exports = function initialize(shim, when) {
  ...
  function wrappedPromise(executor) {
    ...
    if (
      ...
      try {
        ...
        agent.tracer.runInContext({ handler: executor, context: newContext, full: true, thisArg: ctx.self, args: ctx.args })
      } catch (e) {
        ctx.args[1](e)
      }
    }
  ...
  function wrapHandler(ctx, fn, i, arr) {
    ...
    return function __NR_wrappedThenHandler() {
      ...
      let ret = null
      try {
        ret = agent.tracer.runInContext({ handler: fn, context, full: true, thisArg: this, args: arguments })
      } finally {
