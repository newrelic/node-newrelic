...
function initialize(agent, http2, moduleName, shim) {
  ...
  function wrapConnect(shim, fn) {
    return function wrappedConnect(...args) {
      ...
      return instrumentOutboundHttp2(agent, args, function makeRequest(args) {
 ...
 */
function instrumentOutboundHttp2(agent, opts, makeRequest) {
  ...
  return agent.tracer.addSegment(
    ...
    parent,
    false,
    instrumentHttp2Request.bind(null, argArray)
  )
}

function instrumentHttp2Request(
...
function applyHttp2Segment({ opts, makeRequest, host, port, hostname, segment, config }) {
  ...
  const parsed = urltils.scrubAndParseParameters(url.pathname)
