...
function initialize(agent, http2, moduleName, shim) {
  ...
  function wrapRequest(_shim, fn) {
    return function wrappedRequest(...args) {
      ...
      const externalAttrs = extractExternalAttrs({ shim, stream, config: agent.config, headers, authority: this[http2ConnectUrl] })
 ...
 */
function extractExternalAttrs({ shim, config, headers, authority, stream }) {
  ...
  try {
    ...
    shim.logger.trace('Could not parse URL from request headers: %s', err.message)
  }
