    ...
    return function wrappedStreamEmit(...args) {
      ...
      function wrappedEmit(event, arg) {
        ...
        if (isErrorEvent) {
          ...
          handleResponse({ agent, shim, segment, stream: clientStream, response: arg })
        }
 ...
 * @param {Emitter} params.stream wrapped ClientHttp2Stream, the closest analog of request
...
function handleError({ transaction, stream, error }) {
  ...
  // Error events may be emitted multiple times during a transaction, so let's make sure they're unique
  const errorRecorded = errorAlreadyRecorded(transaction, error)
  if (errorRecorded) {
    logger.trace(error, 'Not capturing http2 client error because it has already been captured.')
    return false
  }
...
/**
 * Error events can be emitted several times in the same transaction. Ensure that this one's not duplicated.
 * @param {object} transaction the current transaction
 * @param {object} error the error emitted by the client stream
 * @returns {boolean}
 */
function errorAlreadyRecorded(transaction, error) {
  const currentExceptions = transaction.exceptions.length
  if (currentExceptions === 0) {
    return false
  }
  const latestException = transaction.exceptions[currentExceptions - 1]
  // const codesMatch = error.code === latestException.error?.code
  // const messagesMatch = error.message === latestException.error?.message
  const stacksMatch = error.stack === latestException.error?.stack
  return stacksMatch
}
/**
