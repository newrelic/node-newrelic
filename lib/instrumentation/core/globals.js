  ...
  shim.wrap(process, '_fatalException', function wrapper(shim, original) {
    return function wrappedFatalException(error) {
      ...
      if (!process.domain && !exceptionCallbackRegistered) {
        ...
        // Once synchronous harvesting for networked collectors is implemented,
        // this check can be axed.
        if (agent.config.serverless_mode.enabled) {
          try {
            agent.harvestSync()
          } catch (err) {
            shim.logger.debug(
              'Failed to harvest in the case of an uncaught exception',
              err
            )
          }
        }
