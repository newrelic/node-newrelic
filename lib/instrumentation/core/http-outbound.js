 ...
 * @param {string} library Name of the library being instrumented: either http or http2
 * @returns {object} The instrumented outbound HTTP request.
 */
module.exports = function instrumentOutbound(agent, opts, makeRequest, library = 'http') {
  opts = parseOpts(opts)
  ...
  if (library === 'http2') {
    // we need some extra opts parsing here
    const urlObj = new URL(opts[0])
    // console.log(urlObj)
    argArray.opts = { ...opts, ...urlObj }
  }

  return agent.tracer.addSegment(
    name,
    recordExternal(host, library),
    parent,
    false,
    instrumentRequest.bind(null, argArray)
  )
}
