...
/**
 * Parses, scrubs, and obfuscates a request path.
 *
 * @param {string} requestPath The path to process.
 * @param {object} config The agent configuration.
 * @returns {object} The processed path components.
 */
function parseAndScrubPath(requestPath, config) {
  const parsedUrl = url.parse(requestPath, true)

  const scrubbedPath = urltils.scrub(parsedUrl)
  const queryParams = urltils.parseParameters(parsedUrl)
  const finalPath = urltils.obfuscatePath(config, scrubbedPath)
...
function applySegment({ opts, makeRequest, host, port, hostname, segment, config }) {
  ...
  const parsed = parseAndScrubPath(request.path, config)

  const proto = parsed.protocol || opts.protocol || 'http:'
  segment.name += parsed.finalPath
