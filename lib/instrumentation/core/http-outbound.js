 ...
 * @returns {http.IncomingMessage} request actual http outbound request
 */
function applySegment({ opts, makeRequest, host, port, hostname, segment, config, transaction }) {
  segment.start()
  const request = makeRequest(opts)

  let baseUrl
  // use opts href if available, otherwise fall back to opts.path
  // opts.path can be an absolute url or just a path
  if (!transaction.parsedUrl) {
    if (opts?.href?.includes(request.path)) {
      transaction.parsedUrl = new URL(opts.href)
    } else if (opts.path?.startsWith('http')) {
      transaction.parsedUrl = new URL(opts.path)
    } else {
      const hostPort = hostname ? `${hostname}:${port}` : host
      baseUrl = `${opts.protocol || 'http'}://${hostPort}`
      transaction.parsedUrl = new URL(opts.path || '/', baseUrl)
    }
  }

  const parsed = urltils.scrubAndParseParameters(transaction.parsedUrl)
