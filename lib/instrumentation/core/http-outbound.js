 ...
 */
function instrumentOutbound(agent, opts, makeRequest) {
...
function instrumentRequest(
  ...
  const request = applySegment({
    ...
    segment,
    config: agent.config,
    parsedUrl: transaction.parsedUrl
  })
 ...
 * @param params.parsedUrl
 * @returns {http.IncomingMessage} request actual http outbound request
 */
function applySegment({ opts, makeRequest, host, port, hostname, segment, config, parsedUrl }) {
  segment.start()
  const request = makeRequest(opts)

  // Attempt to parse into new URL.
  // If no href, revert to parsedUrl
  // TODO: just use parsedUrl?
  const urlObject = (opts?.href?.includes(request.path)) ? new URL(opts.href) : parsedUrl
...
module.exports = {
  // TODO: move extractHostPort to urltils?
  extractHostPort,
  instrumentOutbound
}
