...
function initialize(agent, timers, moduleName, shim) {
  ...
  // do that now.
  if (global.setTimeout !== timers.setTimeout && !shim.isWrapped(global.setTimeout)) {
    shim.record(
      global,
      asynchronizers,
      function recordAsynchronizers(shim, fn, name) {
        return {name: 'global.' + name, callback: shim.FIRST}
      }
    )
    ...
    makeWrappedPromisifyCompatible(shim, global)
  }
