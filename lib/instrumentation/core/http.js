  ...
  return function wrappedRequest(input, options, cb) {
    ...
    if (transaction.type === 'web') {
      if (!options.href && !options.path) {
        logger.debug('No href or path, not creating url for transaction %s', transaction.id)
        return
      }
      ...
      if (options.href) {
        ...
        baseUrl = `${options.protocol || 'http'}://${options.hostname || options.host}:${options.port}`
      }
