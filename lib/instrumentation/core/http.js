...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    if (agent.config.instrumentation.http.enabled == false && agent.config.instrumentation.https.enabled == false){
    ...
    response.once('end', instrumentedFinish.bind(response, segment, transaction)) 
...
function instrumentedFinish(segment, transaction) {
  ...
  this.removeListener('end', instrumentedFinish)  // ?
  ...
  //TODO: /api/HttpStreaming become WebTransaction/NormalizedUri/* here, why?
