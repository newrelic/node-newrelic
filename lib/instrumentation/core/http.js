...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    const scrubbedUrl = urltils.scrub(request.url, baseUrl)

    transaction.parsedUrl = new URL(scrubbedUrl, baseUrl)

    // tx url needs to be just the path
...
function instrumentedFinish(segment, transaction) {
  ...
  transaction.finalizeNameFromUri(transaction.parsedUrl.pathname, this.statusCode)
