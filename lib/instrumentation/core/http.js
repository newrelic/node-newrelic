  ...
  shimmer.wrapMethod(http, 'http', 'request', function (request) {
    return agent.tracer.segmentProxy(function wrappedRequest(options, callback) {
      ...
      if (state) {
        ...
        if (callback) {
          requested = agent.errors.monitor(transaction, function () {
            return request.call(this, options, agent.tracer.callbackProxy(callback));
          }.bind(this));
        }
        else {
          requested = agent.errors.monitor(transaction, function (args) {
        ...
        if (!internalOnly) {
          ...
          if (requested.on) requested.on('response', function (res) {
