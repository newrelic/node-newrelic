...
module.exports = function initialize(agent, http) {
  function wrapRequestListener(listener) {
    ...
    return function wrappedHandler(request, response) {
      ...
      return agent.errors.monitor(transaction, function (args) {
  ...
  shimmer.wrapMethod(http, 'http', 'request', function (original) {
    return agent.tracer.segmentProxy(function wrappedRequest(options, callback) {
      ...
      if (state) {
        ...
        if (callback) {
          request = agent.errors.monitor(transaction, function () {
            return original.call(this, options, agent.tracer.callbackProxy(callback));
          }.bind(this));
        }
        else {
          request = agent.errors.monitor(transaction, function (args) {
        ...
        if (callback) {
          /* Even if request isn't in a transaction on start, ensure it will
           * propagate context.
           */
