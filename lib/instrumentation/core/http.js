...
const { extractHostPort, instrumentOutbound } = require('./http-outbound')
...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    const baseUrl = `${transport.toLowerCase()}://${request.headers.host}`

    // the error tracer needs a URL for tracing, even though naming overwrites

    // transaction.scrubbedUrl = urltils.scrub(request.url)
    transaction.parsedUrl = new URL(request.url, baseUrl)
...
function wrapRequest(agent, request) {
  return function wrappedRequest(input, options, cb) {
    ...
    const { host, hostname, port } = extractHostPort(options)

    // Create parsedUrl if it's a web transaction which will be used in instrumentOutbound
    if (transaction.type === 'web') {
      // if (!options.href && !options.path) {
      //   logger.debug('No href or path, not recording for transaction %s', transaction.id)
      ...
      if (options.href) {
        ...
        const hostPort = hostname ? `${hostname}:${port}` : host
        baseUrl = `${options.protocol || 'http'}://${hostPort}`
      }
      transaction.parsedUrl = new URL(options.path, baseUrl)
    }
