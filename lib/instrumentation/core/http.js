...
module.exports = function initialize(agent, http) {
  ...
  shimmer.wrapMethod(http, 'http', 'createServer', function (original) {
    return agent.tracer.transactionProxy(function wrappedCreateServer(actualHandler) {
      agent.environment.setDispatcher('http');

      return original.call(this, agent.tracer.callbackProxy(function (request, response) {
        ...
        if (actualHandler) return actualHandler(request, response);
      }));
    });
  });
