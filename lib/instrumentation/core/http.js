...
function measureExternal(transaction, host, library) {
  if (!host) {
    throw new Error(
  ...
  return function (segment, scope) {
    ...
    transaction.measure(EXTERNAL.ALL, null, duration);

    var rollupName;
    if (segment.trace.transaction.isWeb()) {
      rollupName = EXTERNAL.WEB;
    }
    else {
      rollupName = EXTERNAL.OTHER;
    }
    transaction.measure(rollupName, null, duration);
    transaction.measure(EXTERNAL.PREFIX + host + '/all', null, duration);

    var metricName = EXTERNAL.PREFIX + host + '/' + library;
    transaction.measure(metricName, scope, duration);
    transaction.measure(metricName, null,  duration);
  };
}
...
module.exports = function initialize(agent, http) {
    ...
    return function wrappedHandler(request, response) {
      ...
      if (state && request.url !== '/favicon.ico') {
        ...
        response.once('finish', function instrumentedFinish() {
           ...
           */
          segment.name = transaction.measureWeb(request.originalUrl || request.url,
                                                response.statusCode,
  ...
  shimmer.wrapMethod(http, 'http', 'request', function (request) {
    return agent.tracer.segmentProxy(function wrappedRequest(options, callback) {
      ...
      if (state) {
        ...
        if (!internalOnly) {
          var name     = EXTERNAL.PREFIX + options.host + requested.path
            , gatherer = measureExternal(transaction, options.host, 'http')
  ...
  shimmer.wrapMethod(http, 'http', 'createClient', function (createClient) {
    return agent.tracer.segmentProxy(function wrappedDeprecatedRequest(port, host) {
      ...
      var state = agent.getState();
      if (state) {
        var name        = EXTERNAL.PREFIX + host + request.path
          , transaction = state.getTransaction()
          , gatherer    = measureExternal(transaction, host, 'http')
