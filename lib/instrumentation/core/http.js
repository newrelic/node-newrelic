...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    try {
      ...
      // scrub the URL before assigning it to the transaction
      // when the transaction ends the url will be obfuscated and assigned accordingly
      transaction.url = urltils.scrub(parsedUrl)
    } catch (err) {
    ...
    transaction.applyUserNamingRules(transaction.url)
...
function instrumentedFinish(segment, transaction) {
  ...
  transaction.finalizeNameFromWeb(this.statusCode)
