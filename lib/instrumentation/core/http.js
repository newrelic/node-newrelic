...
function measureExternal(metrics, host, library) {
  if (!host) {
    throw new Error(
  ...
  return function (segment, scope) {
    ...
    metrics.measureDurationUnscoped(EXTERNAL.ALL, duration);
    ...
    metrics.measureDurationUnscoped(rollupName, duration);
    metrics.measureDurationUnscoped(EXTERNAL.PREFIX + host + '/all', duration);

    var metricName = EXTERNAL.PREFIX + host + '/' + library;
    metrics.measureDurationUnscoped(metricName, duration);
    metrics.measureDurationScoped(metricName, scope, duration);
  };
}
      ...
      if (state) {
        ...
        if (!internalOnly) {
          var name     = EXTERNAL.PREFIX + options.host + requested.path
            , gatherer = measureExternal(transaction.metrics, options.host, 'http')
    ...
    return agent.tracer.segmentProxy(function wrappedDeprecatedRequest(port, host) {
      ...
      if (state) {
        var name        = EXTERNAL.PREFIX + host + request.path
          , transaction = state.getTransaction()
          , gatherer    = measureExternal(transaction.metrics, host, 'http')
