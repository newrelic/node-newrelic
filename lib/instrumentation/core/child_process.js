...
const channels = new Map(METHODS.map((name) => [name, tracingChannel('nr_child_process.' + name)]))
...
function rebindStore(agent) {
  ...
  for (const channel of channels.values()) {
    ...
    channel.asyncStart.bindStore(store, (data) => {
      const outerCtx = data.ctx
      if (!outerCtx?.segment) {
        return tracer.getContext()
      }
      outerCtx.segment.touch()

      const segment = createSegment(tracer, outerCtx, 'Callback: ' + data.callbackName)
      if (!segment) {
        return outerCtx
      }
      data.callbackSegment = segment
      return outerCtx.enterSegment({ segment })
    })
  }
