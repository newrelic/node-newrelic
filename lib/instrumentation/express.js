...
function wrapExpress4(shim, express) {
  ...
  wrapExpressExport(shim, express)
}
...
function wrapExpressExport(shim, express) {
  var setDispatcher = shim.once(shim.setDispatcher)
  shim.wrapExport(express, function wrapExport(shim, fn) {
    var wrapper = shim.wrap(fn, function wrapExpress(shim, _fn) {
