...
module.exports = function initialize(agent, express, moduleName, shim) {
  ...
  USE_SPEC = {
    route: shim.FIRST,
    type: function getMiddlwareType(shim, middleware) {
      return middleware.length === 4              ? shim.ERRORWARE
        : middleware.hasOwnProperty('lazyrouter') ? shim.APPLICATION
        : middleware.hasOwnProperty('stack')      ? shim.ROUTER
        : shim.MIDDLEWARE
    }
  }
...
function wrapExpress4(shim, express) {
  var setDispatcher = shim.once(shim.setDispatcher)
  shim.wrap(express.application, 'handle', function wrapHandle(shim, fn) {
    if (!shim.isFunction(fn)) {
      return fn
    }

    return function wrappedAppHandle(req, res) {
      setDispatcher.call(shim)
      shim.requestStarted(req, res, {end: 'end'})
      return fn.apply(this, arguments)
    }
  })

  shim.wrap(express.Router, 'handle', function wrapRouterHandle(shim, fn) {
    if (!shim.isFunction(fn)) {
      return fn
    }

    return function wrappedRouterHandle(req) {
      var args = shim.argsToArray.apply(shim, arguments)
      shim.wrap(args, args.length - 1, function wrapDone(shim, done) {
        if (!shim.isFunction(done)) {
          shim.logger.debug('Could not get callback for Router#handle!')
          return done
        }

        return function wrappedDone(err) {
  ...
  // prototype.
  shim.recordArgsAsMiddleware(express.Router, 'use', USE_SPEC)
  shim.wrap(express.Router, 'route', function wrapRoute(shim, fn) {
    ...
    return function wrappedRoute(path) {
      ...
      if (!shim.isWrapped(route, 'get')) {
        wrapRouteMethods(shim, route, path)
      }
      return route
    }
  })
  ...
  shim.wrapMiddlewareMounter(express.Router, 'param', {
    route: shim.FIRST,
    middleware: function wrapParamware(shim, middleware, fnName, route) {
...
function wrapExpress3(shim, express) {
  ...
  shim.wrapExport(express, function wrapExpress(shim, fn) {
    shim.proxy(fn, Object.keys(fn), wrappedExpress)
    return wrappedExpress
    function wrappedExpress() {
      setDispatcher.call(shim)
      var app = fn.apply(this, arguments)
      shim.wrap(app, 'handle', function wrapAppHandle(shim, handle) {
        return function wrappedAppHandle(req, res) {
          shim.requestStarted(req, res, {end: 'end'})
...
function wrapRouteMethods(shim, route, path) {
  var methods = ['delete', 'get', 'head', 'opts', 'post', 'put', 'patch']
  shim.recordArgsAsMiddleware(route, methods, {
    route: path,
    endpoint: shim.LAST
  })
}
...
function wrapMiddleware(shim, middleware, name, route, isEndpoint) {
  var method = null
  var spec = {
    route: route,
