...
function generateMiddlewarePath(req, layer) {
  var res = ''
  var middlewareList = []
  var currentScope = req.__NR_routerScope

  while (currentScope) {
    middlewareList.unshift(currentScope)
    currentScope = currentScope.parent
  }

  for (var i = 0; i < middlewareList.length; ++i) {
    appendToPath(middlewareList[i].path)
  }
  ...
  // TODO: turn this into prepend so it matches the direction of iteration up
  // the scopes
...
module.exports = function initialize(agent, express) {
    ...
    return function cls_wrapProcessParams(layer) {
      ...
      function wrapLayerHandleError(handleError) {
        return function wrappedLayerHandleError(error, req, res, next) {
          ...
          var transactionInfo = getTransactionInfo(req)
          ...
          if (next) {
            return handleError.call(this, error, req, res, endErrorHandlerRecorder)
          }
          ...
          function endErrorHandlerRecorder(err) {
            if (err) {
              transactionInfo.errorHandled = false
      ...
      function wrapLayerHandleRequest(handleRequest) {
        return function wrappedLayerHandleRequest(req, res, next) {
          ...
          if (agent.config.feature_flag.express_segments) {
            ...
            var currentScope = req.__NR_routerScope
            if (currentScope) {
              parentSegment = currentScope.segment
            }
            ...
            if (!isErrorHandler) {
              ...
              if (isMountedApp) {
                ...
                var middlewarePath = generateMiddlewarePath(req, this)
                recorder = middlewareRecorder.bind(null, middlewarePath)
              }

              var segment = tracer.createSegment(segmentName, recorder, parentSegment)
          ...
          if (isRouter || isRoute || isMountedApp) {
            req.__NR_routerScope = {
              path: this.handle.__NR_path,
              segment: segment,
              parent: req.__NR_routerScope
            }
          } else {
            transactionInfo.lastMiddlewareSegment = segment

            // update transaction name based on the latest visited middleware
            // it this was not done, transactions with errors would never
            // get named because the router stack gets unwound when an error occurs
            transaction.partialName = null
            ensurePartialName(transaction)
            var path = generateMiddlewarePath(req, this)
            if (path[0] === '/') path = path.substring(1)
            transaction.partialName += path
          }

          if ((isRouter || isRoute) && req.params) {
          ...
          if (next) {
            return handleRequest.call(this, req, res, wrappedNext)
          }
          ...
          function wrappedNext(err) {
            ...
            if (!transactionInfo.responded && (isMountedApp || isRouter || isRoute)) {
              parentScope = req.__NR_routerScope.parent
              req.__NR_routerScope = parentScope
            }
  ...
  function wrapHandle(__NR_handle, path) {
    ...
    var wrapped = new Function(
      'tracer', '__NR_handle', 'path', 'template',
  ...
  function getTransactionInfo(req) {
    if (!req.__NR_transactionInfo) {
      req.__NR_transactionInfo = {
