...
function wrapRegister(shim, original) {
  ...
  return function wrappedRegister(...args) {
    ...
    function instrumentedHandler(stream) {
      ...
      // must bind the `.emit` of stream, otherwise the context will be broken
      // in the stream event handlers
      stream.emit = shim.tracer.bindFunction(stream.emit, ctx)
      ...
      return shim.applyContext({ func: handler, context: ctx, full: true, boundThis: this, args: arguments })
    }
    ...
    function createTransaction() {
      ...
      const newCtx = context.enterSegment({ segment })
