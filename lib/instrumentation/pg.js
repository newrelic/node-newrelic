      ...
      shimmer.wrapMethod(obj.nodule, obj.english, 'Client', function wrapper(Client) {
        ...
        function wrapClient() {
          ...
          function nativeQueryWrapper(segment, args, bindCallback) {
            ...
            captureInstanceAttributes(segment, client)
...
function captureInstanceAttributes(segment, client) {
  if (client.host && client.port) {
    segment.host = client.host
    segment.portPathOrId = client.port
    segment.databaseName = client.database
    segment.parameters.instance = client.host + ':' + client.port
  }
}
