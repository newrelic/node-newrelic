 ...
 *
 * @param {object} response The OpenAI API response object
...
function getMessageFromResponse(response) {
  const message = { content: undefined, role: undefined }
  if (response?.object === 'response') {
    message.content = response?.output?.[0]?.content?.[0]?.text
    message.role = response?.output?.[0]?.role
  } else {
 ...
 *
 * @param {object} request The OpenAI API request object
 * @returns an array of message objects with fields `content` and `role`
 */
function getMessagesFromRequest(request) {
  if (Array.isArray(request?.input)) {
    // `responses.create` logic
    return request.input.map((msg) => ({
      content: msg.content,
      role: msg.role
    }))
    ...
    // single message case for input
    return [{
      content: request.input,
      role: 'user'
    }]
  } else if (request?.message) {
    // single message case for message
    return [{
...
function recordChatCompletionMessages({
  ...
  const messages = getMessagesFromRequest(request)
  const responseMessage = getMessageFromResponse(response)
  if (responseMessage.content !== undefined || responseMessage.role !== undefined) {
...
module.exports = function initialize(agent, openai, moduleName, shim) {
  ...
  if (semver.gte(shim.pkgVersion, '5.0.0')) {
    shim.record(openaiClient.Responses.prototype, 'create', function wrapCreate(shim, create, name, args) {
      ...
      if (request.stream) {
        shim.logger.warn(
          'Instrumentation for `responses.create` streams is not yet supported.'
        )
        return
      }
