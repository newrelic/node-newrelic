 ...
 */
function recordEvent({ agent, segment, type, msg }) {
  const context = segment.transaction._llmContext.getStore()
  agent.emit('recordLlmEvent', type, msg, context)
...
function recordChatCompletionMessages({ agent, shim, segment, request, response, err }) {
  ...
  messages.forEach((message, index) => {
    ...
    recordEvent({ agent, segment, type: 'LlmChatCompletionMessage', msg: completionMsg })
  })

  recordEvent({ agent, segment, type: 'LlmChatCompletionSummary', msg: completionSummary })
