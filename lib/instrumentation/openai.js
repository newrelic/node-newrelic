...
function recordChatCompletionMessages({
  ...
  if (response?.object === 'response') {
    ...
    // request.input can be a single string or an array of messages
    if (Array.isArray(request.input)) {
      messages = [...request.input, response?.output?.[0]]
    } else messages = [request.input, response?.output?.[0]]
    ...
    return async function * wrappedIterator() {
      ...
      try {
        ...
        if (chunk.choices) {
          ...
          // Note about `responses.create`:
          // The final chunk has the response object
          // in chunk.response, so we'll just use that
          // as our response for recordChatCompletionMessages
