...
module.exports = function initialize(agent, mysql) {
  ...
  function wrapQueriable(queriable, name) {
    ...
    function cb_wrapMethod(segment, args, bind) {
      ...
      if (args.length === 1 && typeof args[0] === 'object') {
        ...
        args[0]._callback = bind(args[0]._callback, true, true)
        ...
        if (typeof args[0] !== 'function') {
          sqlString = args[0].sql
          vargs.push(args[0])
          vargs.push(bind(null, true, true))
        } else {
          vargs.push(bind(args[0]. true, true))
        }
      ...
      var segmentName = MYSQL.STATEMENT + (ps.model || 'unknown') + '/' + ps.operation
  ...
  if (mysql && mysql.createConnection) {
    ...
    shimmer.wrapMethod(mysql, 'mysql', 'createConnection', function cb_wrapMethod(createConnection) {
      return function wrappedCreateConnection() {
        ...
        shimmer.wrapMethod(connection, 'connection', 'query', function(original) {
          return tracer.wrapFunction(
            MYSQL.STATEMENT + 'Unknown',
            null,
            original,
        ...
        function wrappedQuery(segment, args, bind) {
          ...
          var wrapped = bind(actualCallback)
          ...
          if (agent.config) {
            segment.port = agent.config.port
            segment.host = agent.config.host
          }

          logger.trace('Adding node-mysql 2 query trace segment on transaction %s.',
                       transaction.id)

          return [sql, actualValues, wrapped]
        }
  ...
  function wrapQuery09(segment, args, bind) {
    ...
    logger.trace('Tracing node-mysql 0.9 query on transaction %s.',
                 transaction.id)
