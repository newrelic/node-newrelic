...
function queryParser(params) {
  ...
  let queryParam = {}
  if (isSimpleObject(params.querystring) && Object.keys(params.querystring).length > 0) {
    queryParam = params.querystring
  }
  // let body or bulkBody override querystring, as some requests have both
  if (isSimpleObject(params.body) && Object.keys(params.body).length > 0) {
    queryParam = params.body
  } else if (isSimpleObject(params.bulkBody) && Object.keys(params.bulkBody).length > 0) {
