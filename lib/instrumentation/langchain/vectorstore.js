    ...
    function wrapCall(shim, similaritySearch, fnName, args) {
      ...
      return new RecorderSpec({
        ...
        after(_shim, _fn, _name, _err, output, segment) {
          segment.end()
          const vectorstoreEvent = new LangChainVectorSearch({
            agent,
            query: request,
            k: params,
            documents: output
          ...
          recordEvent({ agent, type: 'LlmVectorstoreSearch', pkgVersion, msg: vectorstoreEvent })

          output.forEach((document, sequence) => {
            const documentResult = new LangChainVectorSearchResult({
              agent,
            ...
            recordEvent({
              agent,
              type: 'LlmVectorstoreSearchResult',
              pkgVersion,
              msg: documentResult
            })
          })
