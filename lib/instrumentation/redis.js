...
module.exports = function initialize(_agent, redis, _moduleName, shim) {
  const version = Number(shim.pkgVersion.split('.').shift())
  if (version < 5) {
    return registerV3ThroughV4(_agent, redis, _moduleName, shim)
  }

  if (version === 5) {
    shim.logger.info('Skipping redis instrumentation as v5 is supported through @redis/client')
    return
  }

  shim.logger.warn('Skipping redis instrumentation due to unrecognized module version')
}

function registerV3ThroughV4(_agent, redis, _moduleName, shim) {
