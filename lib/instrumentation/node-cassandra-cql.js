...
var logger = require('../logger').child({component: 'node-cassandra-cql'})
...
module.exports = function initialize(agent, cassandracql) {
  ...
  INSTRUMENTED_OPERATIONS.forEach(function cb_forEach(operation) {
    ...
    function wrapper(segment, args, bind) {
      ...
      logger.trace(
        'Adding cassandra-cql command trace segment transaction %s.',
        transaction.id
      )
      ...
      if (typeof last === 'function') {
        ...
        last[last.length - 1] = tracer.bindFunction(bind(last[last.length - 1], true, true))
