...
function wrapCreateContext(shim, fn, fnName, context) {
  ...
  Object.defineProperty(context.response, '_body', {
    ...
    set: function setBody(val) {
      if (context.matched) {
        // for matched where end === true
        // grab item w/ lowest paramNames.length
         console.log('matched:', context.matched)
        // TODO: can prob single pass this
        const matched = context.matched
          .filter((l) => l.opts.end)
          .sort((a, b) => a.paramNames.length - b.paramNames.length)
        console.log('path:', matched[0].path)
        // console.log('* path', path[0].path)

        if (!context.__NR_bodySet) {
          // shim.setTransactionUri(context.request.url) // called once per transaction

          const tx = shim.agent.tracer.getTransaction()
          tx.nameState.appendPath(matched[0].path)
          tx.nameState.markPath()
          tx.nameState.freeze()

          console.log('setting txUri:', matched[0].path)
          // shim.savePossibleTransactionName(matched[0].path)
          // shim.setTransactionUri(matched[0].path)
        } else {
      ...
      //   console.log('matched:', context.matched)
        // shim.setTransactionUri(context.matched[0].path)
