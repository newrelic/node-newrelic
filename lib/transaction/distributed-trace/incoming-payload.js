...
class IncomingPayload {
  ...
  #isDtAlreadyRecorded() {
    ...
    // We are deliberately using a "truthy" check here (`if (foo)`) as
    // opposed to a coercive value check (`if (foo != null)`). We are not aware
    // of any edge cases the coercive check would fail with, but the agent has
    // historically used a truthy check here, so we propagate it to avoid bugs.
    const supportabilityMetric = this.#transaction.parentId
