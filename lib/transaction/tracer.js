...
Tracer.prototype.createState = function (transaction, segment, handler) {
  ...
  if (state.call) state.call = namespace.bind(state.call, context);
...
Tracer.prototype.transactionProxy = function (handler) {
  ...
  var wrapped = function wrapTransactionInvocation() {
    ...
    var state = self.createState(transaction, transaction.getTrace().root, handler);
    transaction.context = state.context;

    return self.monitor(state.call, this, arguments, transaction);
  };
...
Tracer.prototype.segmentProxy = function (handler) {
  ...
  var wrapped = function wrapSegmentInvocation() {
    ...
    var newState = self.createState(state.transaction, state.segment,
                                    handler, state.domain);
    return self.monitor(newState.call, this, arguments, newState.transaction);
  };
  wrapped[ORIGINAL] = handler;
...
Tracer.prototype.callbackProxy = function (handler) {
  ...
  var wrapped = function wrapCallbackInvocation() {
    var newState = self.createState(state.transaction, state.segment, handler);
    return self.monitor(newState.call, this, arguments, newState.transaction);
  };
  wrapped[ORIGINAL] = handler;
