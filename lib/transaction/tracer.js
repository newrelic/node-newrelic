...
// namespace-based state
function State(transaction, segment, call) {
  ...
  namespace.set('call', call);
  namespace.set('state', this);
}
State.prototype.getTransaction = function () { return namespace.get('transaction'); };
State.prototype.getSegment = function () { return namespace.get('segment'); };
State.prototype.setSegment = function (segment) { namespace.set('segment', segment); };
State.prototype.getCall = function () { return namespace.get('call'); };
State.prototype.setCall = function (call) { namespace.set('call', call); };
...
function Tracer(agent) {
  ...
  if (callbacks && callbacks.error) {
    callbacks.error = function (domain, error) {
      var transaction = domain && domain.state && domain.state.getTransaction();
...
Tracer.prototype.createState = function (transaction, segment, handler) {
  return new State(transaction, segment, handler);
};
...
Tracer.prototype.transactionProxy = function (handler) {
  ...
  var wrapped = function wrapTransactionInvocation() {
    ...
    namespace.bind(function () {
      self.createState(transaction, transaction.getTrace().root, handler);
