...
const REQUIRED_DISTRIBUTED_TRACE_KEYS = ['ty', 'ac', 'ap', 'tr', 'ti']
 ...
 */
Transaction.prototype.getIntrinsicAttributes = getIntrinsicAttributes
function getIntrinsicAttributes(addParentData) {
  if (!this._intrinsicAttributes.totalTime) {
    ...
    if (config.cross_application_tracer.enabled) {
      if (config.feature_flag.distributed_tracing) {
        this._addDistributedTraceIntrinsics(this._intrinsicAttributes, addParentData)
      } else {
...
function acceptDistributedTracePayload(payload) {
  ...
  const allKeysExist = REQUIRED_DISTRIBUTED_TRACE_KEYS.every(function checkExists(key) {
    return data[key] != null
  })
  // Either parentSpanId or parentId are required.
  if (!allKeysExist || data.tx == null && data.id == null) {
...
function _addDistributedTraceIntrinsics(attrs, isTransactionEvent) {
  this._calculatePriority()
  ...
  if (this.parentSpanId && isTransactionEvent) {
    attrs.parentSpanId = this.parentSpanId
  }

  if (this.parentId && isTransactionEvent) {
