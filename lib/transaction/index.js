...
const _dtConfigTest = function _dtConfigTest() {
  ...
  const trustedAccount = config.distributed_tracing.trusted_account_key || config.distributed_tracing.account_id
...
function _createDistributedTracePayload() {
  const config = this.agent.config
  const accountId = config.distributed_tracing.account_id
  const appId = config.distributed_tracing.primary_application_id
  const distTraceEnabled = config.distributed_tracing.enabled
  ...
  if (config.distributed_tracing.trusted_account_key && config.distributed_tracing.trusted_account_key !== accountId) {
    data.tk = config.distributed_tracing.trusted_account_key
  }
