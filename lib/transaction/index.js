 ...
 *
 * @private
 * @param {string} scrubbedUrl - The scrubbed URL to derive the name from.
...
function _partialNameFromUri(scrubbedUrl, status) {
 ...
 *
 * @param {url.URL} requestURL - The URL to derive the request's name and status from.
...
function finalizeNameFromUri(requestURL, statusCode) {
  ...
  // TODO: should we just get this from tx.parsedUrl?
 ...
 *
 * @param {url.URL} requestURL The URL, as it came in, for parameter extraction.
 */
Transaction.prototype._markAsWeb = function _markAsWeb(requestURL) {
  // Because we are assured we have the URL here, lets grab query params.
  const params = urltils.parseParameters(requestURL)
  for (const key in params) {
    if (props.hasOwn(params, key)) {
...
Transaction.prototype.finalizeName = function finalizeName(name) {
  ...
  const url = this.parsedUrl?.pathname || this.url
  if (name == null && this.isWeb() && url) {
    return this.finalizeNameFromUri(url, this.statusCode)
  }
...
Transaction.prototype.getName = function getName() {
  const url = this.parsedUrl ? this.parsedUrl.pathname : this.url
  if (this.isWeb() && url) {
    const finalName = this._partialNameFromUri(url, this.statusCode)
    if (finalName.ignore) {
      this.ignore = true
    }
