...
Transaction.prototype._setApdex = function _setApdex(name, duration, keyApdexInMillis) {
  ...
  if (
    ...
    // and error on the side of incrementing frustrating.  If this
    // is an issue, users can either set a status code as expected,
    // or ignore the specific error to avoid incrementing to frustrating
    !this.allErrorsExpected()
  ) {
 ...
 * are expected errors.
 */
Transaction.prototype.allErrorsExpected = function allErrorsExpected() {
  for (let i = 0;i < this.exceptions.length;i++) {
    const exception = this.exceptions[i]
    if (!errorHelper.isExpectedException(this, exception[0], this.agent.config, urltils)) {
      return false
    }
  }
  return true
