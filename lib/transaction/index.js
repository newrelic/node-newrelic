...
function Transaction(agent) {
  ...
  this.traceStacks = new TraceStacks(agent.config)
  this.agent = agent
  ...
  this.traceStacks.probe('Transaction created', { id: this.id })
}
...
Transaction.prototype.end = function end() {
  ...
  if (this.traceFlag) {
    logger.warn(this.traceStacks.serialize(this.name), 'Flagged transaction ended.')
  }
