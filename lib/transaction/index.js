  ...
  if (!this._intrinsicAttributes.totalTime) {
    ...
    if (config.cross_application_tracer.enabled) {
      if (config.feature_flag.distributed_tracing) {
        // TODO: pull off all the relevant data here https://source.datanerd.us/earnold/agent-specs/blob/dd291549d8af103d64ee6fba061a3c19cd23cc70/Distributed-Tracing.md#attributes
      } else {
...
function acceptDistributedTracePayload(payload) {
  if (!payload) {
    this.agent.recordSupportability(
      'Transactions/DistributedTrace/AcceptPayload/Ignored/Null'
    )
    return
  }
  ...
  if (this.isDistributedTrace) {
    ...
    if (this.parentId) {
      this.agent.recordSupportability(
        'Transactions/DistributedTrace/AcceptPayload/Multiple'
      )
    } else {
      this.agent.recordSupportability(
        'Transactions/DistributedTrace/AcceptPayload/CreateBeforeAccept'
      )
    }
    return
  }
  ...
  if (typeof payload === 'string') {
    if (payload.charAt(0) !== '{' && payload.charAt(0) !== '[') {
      try {
        payload = hashes.deobfuscateNameUsingKey(payload, this.agent.config.encoding_key)
      } catch (e) {
        logger.warn(
    ...
    try {
      ...
      this.agent.recordSupportability(
        'Transactions/DistributedTrace/AcceptPayload/ParseException'
      )
      return
    }
  }
  ...
  if (majorVersion > 0) { // TODO: Add DistributedTracePayload class?
    this.agent.recordSupportability(
      'Transactions/DistributedTrace/AcceptPayload/MajorVersion'
    )
    return
  }

  const data = parsed.d || Object.create(null)

  const accountId = data.ac
  if (trustedAccts.indexOf(accountId) === -1) {
    this.agent.recordSupportability(
      'Transactions/DistributedTrace/AcceptPayload/UntrustedAccount'
    )
    return
  }
  ...
  this.priority = data.pr || this.priority
  this.sampled = !!data.sa // TODO: https://source.datanerd.us/earnold/agent-specs/blob/dd291549d8af103d64ee6fba061a3c19cd23cc70/Distributed-Tracing.md#sampled

  this.isDistributedTrace = true

  this.agent.recordSupportability('Supportability/DistributedTrace/AcceptPayload/Success')
}
...
function createDistributedTracePayload() {
  ...
  const data = {
    ...
    ap: appId,
    id: this.id,
    tr: '', // TODO: https://source.datanerd.us/earnold/agent-specs/blob/389ba868afe2d3179c75e96d6d47c8ec3b253c3e/Distributed-Tracing.md#traceid
    pr: this.priority,
    sa: false, // TODO: https://source.datanerd.us/earnold/agent-specs/blob/dd291549d8af103d64ee6fba061a3c19cd23cc70/Distributed-Tracing.md#sampled
    ti: Date.now()
  }

  if (true /* something */ ) {
    data.pa = '' // TODO: something
  }

  this.isDistributedTrace = true

  this.agent.recordSupportability('Supportability/DistributedTrace/CreatePayload/Success')
