...
class Transaction {
  ...
  static TRANSPORT_TYPES = Transport
  static TYPES = Type

  #logger

  // The remaining _ prefixed methods (e.g. _acceptDistributedTracePayload,
  // _createDistributedTracePayload) cannot become #private yet because unit
  // tests call them directly; reroute those tests through the public entry
  // points first.
  ...
  _acceptDistributedTracePayload(payload, transport) {
    const dtTracePayload = new IncomingPayload({
      agent: this.agent,
      logger: this.#logger,
      transaction: this
    })
   ...
   *
   * @param {object} attrs - The intrinsics object to add distributed trace attributes to.
   */
   ...
   *                            metrics. Takes a single parameter, which is the
   *                            transaction's name.
   */
