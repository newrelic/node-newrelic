...
function acceptDistributedTracePayload(payload) {
  ...
  this.parentTransportDuration = Date.now() / 1000 - data.ti
...
function acceptDistributedTracePayload(payload) {
  ...
  const distTraceEnabled = this.config.feature_flag.distributed_tracing
  const catEnabled = this.config.cross_application_tracer.enabled
  const trustedAccts = this.config.trusted_account_ids
