...
Transaction.prototype._markAsWeb = function _markAsWeb(rawURL) {
  ...
  // so I'm not worried about `attributes.exclude` or `attributes.enabled`.
  var params = util._extend({}, this.baseSegment.parameters)
  ...
  urltils.copyParameters(this.agent.config, urltils.parseParameters(rawURL), params)
...
Transaction.prototype.addAgentAttribute = function addAgentAttribute(key, value) {
  if (this.agent.config.attributes.exclude.indexOf(key) === -1) {
