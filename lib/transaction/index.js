...
const DISTRIBUTED_TRACE_KEYS = ['ty', 'ac', 'ap', 'id', 'tr', 'ti']
...
Transaction.prototype.computeSampled = function computeSampled() {
  if (this.sampled !== null) {
    return this.sampled
  }

  this.sampled = this.agent.shouldSampleDistributedTrace(this)
    ...
    this.nameState.forEachParams(function forEachRouteParams(params) {
      for (var key in params) {
        if (props.hasOwn(params, key)) {
          this.trace.addAttribute(DESTS.NONE, 'request.parameters.' + key, params[key])
        }
      }
    }, this)
  }
  ...
  if (!this._intrinsicAttributes.totalTime) {
    ...
    if (config.cross_application_tracer.enabled) {
      if (config.feature_flag.distributed_tracing) {
        this._addDistributedTraceIntrinsics(this._intrinsicAttributes)
      } else {
...
function acceptDistributedTracePayload(payload) {
  if (!payload) {
  ...
  const distTraceEnabled = this.agent.config.feature_flag.distributed_tracing
  const catEnabled = this.agent.config.cross_application_tracer.enabled
  const trustedAccts = this.agent.config.trusted_account_ids

  if (!distTraceEnabled || !catEnabled || !trustedAccts.length) {
  ...
  const allKeysExist = DISTRIBUTED_TRACE_KEYS.every(function checkExists(key) {
  ...
  const accountId = data.ac
  if (trustedAccts.map(String).indexOf(accountId) === -1) {
    this.agent.recordSupportability('DistributedTrace/AcceptPayload/UntrustedAccount')
    return
  }

  this.grandParentId = data.pa || null
  this.parentType = data.ty
  this.parentId = data.id
  this.parentApp = data.ap
  this.parentAcct = accountId
  this.parentTransportType = 'http'
  this.parentTransportDuration = (Date.now() - data.ti) / 1000
  this.traceId = data.tr
  if (data.pr) {
    this.priority = data.pr
    var oldSampled = this.sampled
    this.sampled = data.sa != null ? data.sa : this.sampled
    // The case where oldSampled and data.sa should never happen since
    // it would require sampling a transaction outside of a distributed
    // trace, but we check anyway just to be safe.
    if (!oldSampled && data.sa === true) {
      ++this.agent.transactionsSampled
    }
  }
...
Transaction.prototype._getParsedPayload = function _getParsedPayload(payload) {
  ...
  if (typeof payload === 'string') {
    if (payload.charAt(0) !== '{' && payload.charAt(0) !== '[') {
      try {
        payload = hashes.makeBuffer(payload, 'base64').toString()
      } catch (err) {
        logger.warn(
          err,
...
function createDistributedTracePayload() {
  ...
  const ids = config.cross_process_id.split('#')
  const accountId = ids[0]
  const appId = config.application_id || ids[1]
  ...
  // This is side-effectful (it adds 1 to the priority if it is sampled),
  // so we call it before constructing the data object.
  var sampled = this.computeSampled()
  const data = {
    ty: 'App',
    ac: accountId,
    ap: appId,
  ...
  return JSON.stringify({v: [0,1], d: data})
}

/**
 * Adds distributed trace attributes to instrinsics object.
 */
Transaction.prototype._addDistributedTraceIntrinsics = _addDistributedTraceIntrinsics

function _addDistributedTraceIntrinsics(attrs) {
  // *always* add these if DT flag is enabled
  attrs.traceId = attrs['nr.tripId'] = this.traceId || this.id
  attrs.guid = this.id
  attrs.priority = this.priority

  // In case we haven't computed this for some reason. This should have
  // no effect if called multiple times.
  if (this.sampled == null) {
    throw new Error('_addDistributedTraceIntrinsics requires a boolean sampled state')
  }
  ...
  if (this.grandParentId) {
    attrs.grandparentId = this.grandParentId
  }

  if (this.parentId) {
    attrs.parentId = this.parentId
  }
}
