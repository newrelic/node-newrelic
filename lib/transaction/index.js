...
Transaction.prototype._markAsWeb = function _markAsWeb(rawURL) {
  ...
  for (var key in params) { // eslint-disable-line guard-for-in
    var attrKey = 'request.parameters.' + key
    if (props.hasOwn(params, key) && !this.trace.attributes.has(attrKey)) {
      this.trace.addAttribute(DESTS.NONE, attrKey, params[key])
    }
  }
