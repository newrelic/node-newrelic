...
const TraceIdRatioBasedSampler = require('../samplers/ratio-based-sampler')
...
function decideSamplingFromW3cData({ transaction, traceparent, tracestate }) {
  ...
  if (traceparent.isSampled === true) {
    applySamplingStrategy({ transaction, tracestate, sampler: parentSampled })
  } else if (traceparent.isSampled === false) {
    applySamplingStrategy({ transaction, tracestate, sampler: parentNotSampled })
  }
}
 ...
 * @param {string|object} params.sampler A string describing the sampler being used
 * ('default', 'always_on', 'always_off') or an object for 'trace_id_ratio_based'.
 */
function applySamplingStrategy({ transaction, tracestate, sampler }) {
  const ratio = sampler?.trace_id_ratio_based?.ratio
  // This slightly complicated boolean check is for the use case
  // where the customer sets `ratio` to 0, which is a falsey value.
  const validRatio = (typeof ratio === 'number' && ratio >= 0 && ratio <= 1)
  if (validRatio) {
    const tempSampler = new TraceIdRatioBasedSampler({
      agent: transaction.agent,
      ratio: sampler.trace_id_ratio_based.ratio
    })
    transaction.sampled = tempSampler.shouldSample(transaction.traceId)
    // TODO: May have to update the priority if the `sampled` value changed?
...
Transaction.prototype._calculatePriority = function _calculatePriority() {
  if (this.priority === null) {
    ...
    const shouldSampleParam = this.agent.transactionSampler.toString() === 'TraceIdRatioBasedSampler'
      ? this.traceId
      // We want to separate the priority roll from the decision roll to
      // avoid biasing the priority range
      // eslint-disable-next-line sonarjs/pseudo-random
      : Math.random()
    this.sampled = this.agent.transactionSampler.shouldSample(shouldSampleParam)
    if (this.sampled) {
      this.priority += 1
    }
