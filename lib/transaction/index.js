...
Transaction.prototype.end = function end() {
  ...
  // If no sampling decision has been made, make it now
  if (this.priority === null) {
    this.agent.sampler.root.applySamplingDecision({ transaction: this })
  }
...
function acceptTraceContextPayload(traceparentHeader, tracestateHeader, transport) {
  ...
  if (traceparent.isSampled === true) {
    this.agent.sampler.remoteParentSampled.applySamplingDecision({ transaction: this, tracestate })
  } else if (traceparent.isSampled === false) {
    this.agent.sampler.remoteParentNotSampled.applySamplingDecision({ transaction: this, tracestate })
  }
}
