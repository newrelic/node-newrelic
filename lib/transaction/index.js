...
const IncomingPayload = require('./distributed-trace/incoming-payload.js')
const OutgoingPayload = require('./distributed-trace/outgoing-payload.js')
const { PayloadData } = require('./distributed-trace/payload.js')
const OutgoingPayloadStub = OutgoingPayload.Stub
const PartialTrace = require('./trace/partial-trace.js')
const Type = require('./type.js')
const Transport = require('./distributed-trace/transport.js')
...
class Transaction {
  ...
  #logger
  // Backing field for the lazily-generated `traceId`. Left null until first
  // read (or an explicit set) so transactions that never read it don't pay the
  // id-generation cost.
  #traceId
   ...
   */
  #cleanUnneededReferences() {
    this.userErrors = null
    this.exceptions = null
  }

  #copyNameToActiveSpan(name) {
  ...
  /**
   * Creates a distributed trace payload utilizing the information present
   * in the current transaction. Used internally when adding distributed
   * trace header to the transaction.
   *
   * @returns {OutgoingPayload|OutgoingPayloadStub} The distributed trace
   * payload, or a stub when distributed tracing is not adequately configured.
   */
  #createDistributedTracePayload() {
    ...
    if (!accountId || !appId || !distTraceEnabled) {
      ...
      return new OutgoingPayloadStub()
    }

    const currSegment = this.agent.tracer.getSegment()
    const data = new PayloadData({
      accountId,
      appId,
      type: PayloadData.TYPE_APP,
   ...
   * @param {TraceSegment} segment The segment to which the exception is linked.
   */
  #linkExceptionToSegment(exception, segment) {
  ...
  /**
   * Derive the transaction partial name from `this.url` and `this.statusCode`.
   *
  ...
  #partialNameFromUri() {
    ...
    // naming from API
    const userNormalized = this.#runUserNamingRules(scrubbedUrl)
  ...
  /**
   * Executes the user and server provided naming rules to clean up the given
   * url.
   *
   * @param {string} requestUrl The URL to normalize.
   * @returns {object} The normalization results after running user and server
   * rules.
   */
  #runUserNamingRules(requestUrl) {
  ...
  /**
   * Parses incoming headers for distributed trace information and applies
   * the results to the current transaction. W3C TraceContext format is
   * preferred over the NewRelic DT format. NewRelic DT format will be used if
   * no `traceparent` header is found.
   *
   * @param {string} transportType The transport type that delivered the trace.
   * @param {object} headers Headers to search for supported trace formats.
  ...
  acceptDistributedTraceHeaders(transportType, headers) {
    ...
    const transport = Transport.isValid(transportType) ? transportType : Transport.UNKNOWN
  ...
  /**
   * Copies distributed trace attributes from the transaction instance
   * to the supplied attributes object. Typically, the object passed in is
   * an instance of `Object.create(null)`.
   *
   * @param {object} attrs The intrinsics object to add distributed trace
   * attributes to.
   */
  ...
  /**
   * Handles accepting upstream DT headers(traceparent/tracestate or newrelic)
   * if present and DT is enabled.
   *
   * @param {object} params Method parameters.
   * @param {object} params.headers HTTP headers of the request(must be
   * lowercased, by default http instrumentation has headers lower case, other
   * libraries must pass this in lowercased).
   * @param {string} [params.transport] Transport type that delivered the
   * request. Use the static values available on {@link Transport}.
   */
  ...
  addException(exception, segment) {
    ...
    this.#linkExceptionToSegment(exception, segment)
   ...
   *
   * @param {function(string): void} recorder The callback which records
   * metrics. Takes a single parameter, which is the transaction's name.
   */
  ...
  /**
   * Associate a user error (reported using the noticeError() API) with the
   * transaction. When the transaction ends, the exception will be collected
  ...
  addUserError(exception) {
    ...
    this.#linkExceptionToSegment(exception)
   ...
   *
   * @param {string} requestUrl The URL to normalize and apply to this
   * transaction.
   */
  applyUserNamingRules(requestUrl) {
    const normalized = this.#runUserNamingRules(requestUrl)
   ...
   *
   * @param {string} [name] The partial name to use for the finalized
   * transaction. If omitted the current partial name is used.
   */
  finalizeName(name) {
    ...
    if (name == null && this.isWeb() && this.url) {
      this.finalizeNameFromWeb(this.statusCode)
      return
    }
    ...
    this.#copyNameToActiveSpan(this.name)
  ...
  getName() {
    if (this.isWeb() && this.url) {
      const finalName = this.#partialNameFromUri()
      if (finalName.ignore) {
        this.ignore = true
      }
  ...
  /**
   * @returns {boolean} true if an error happened during the transaction or if
   * the transaction itself is considered to be an error.
   */
  hasErrors() {
   ...
   *
   * @param {object} headers The headers map to insert distributed trace headers into.
   ...
   *
   * @returns {boolean} May be a truthy value.
   */
  isIgnored() {
   ...
   *
   * @returns {boolean} True when the transaction is a web transaction.
   ...
   * @param {number} keyApdexInMillis Duration sent to the metrics
   * getOrCreateApdexMetric method, to derive apdex from timing in milliseconds.
   */
  setApdex(name, duration, keyApdexInMillis) {
    if (this.ignoreApdex) {
      this.#logger.trace(
