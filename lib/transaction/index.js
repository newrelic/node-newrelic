...
function finalizeNameFromUri(requestURL, statusCode) {
  ...
  if (!this.agent.config.high_security && !this.nameState.isEmpty() && this.baseSegment) {
    this.nameState.forEachParams(function forEachRouteParams(params) {
      for (var key in params) {
        if (properties.hasOwn(params, key)) {
          this.trace.addAttribute('request.parameters.' + key, params[key])
        }
      }
    }, this)
  }
...
Transaction.prototype._markAsWeb = function _markAsWeb(rawURL) {
  // Because we are assured we have the URL here, lets grab query params. We
  // want to opt for keeping the keys that are already on params, so we use
  // `copyParameters`.
  //
  // TODO: In v4, these parameters should be prefixed with `request.parameters.`
  this.trace.addAttributes(urltils.parseParameters(rawURL))
...
Transaction.prototype.getScrubbedUrl = function getScrubbedUrl() {
  ...
  var scrubbedParsedUrl = util._extend(Object.create(null), this.parsedUrl)
...
Transaction.prototype.addAgentAttribute = function addAgentAttribute(key, value) {
  this.trace.addAttribute(key, value)
}
...
Transaction.prototype.getIntrinsicAttributes = function getIntrinsicAttributes() {
  ...
  return util._extend(Object.create(null), this._intrinsicAttributes)
}
