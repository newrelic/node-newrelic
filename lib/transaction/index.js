...
function Transaction(agent) {
  ...
  this.priority = null
  this.sampled = null
  this.traceContext = Object.create(null)
...
function acceptTraceContextPayload(parent) {
  if (this._validateTraceParentHeader(parent)) {
    // TODO: add supporability metrics for accepting header
    this.traceContext.parent = parent
  }

  // then acceptDTPayload
}

Transaction.prototype._validateTraceParentHeader = 
function _validateTraceParentHeader(header) {
  const rgx = /^[a-f0-9]{2}-(?![0]{32})[a-f0-9]{32}-(?![0]{16})[a-f0-9]{16}-[a-f0-9]{2}$/
 ...
 */
// TODO:  might have to do the tracecontext check propagation here?
...
Transaction.prototype.createTraceContextHeaders = createTraceContextHeaders

function createTraceContextHeaders() {
  return {
    parent: this.createTraceParentHeader(),
    state: { todo: 'makethis' }
  }
}

Transaction.prototype.createTraceParentHeader = createTraceParentHeader

function createTraceParentHeader() {
  let traceId
  // TODO: maybe more to helper function
  if (this.traceContext.parent && 
    this._validateTraceParentHeader(this.traceContext.parent)) {
    traceId = this.traceContext.parent.substr(3, 32)
  } else {
    traceId = this.id + hashes.makeId()
  }

  const version = '00' // TODO: constant?
  let parentId = this.agent.config.span_events.enabled ? 
    this.agent.tracer.getSegment().id :
