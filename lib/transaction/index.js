...
function acceptTraceContextPayload(traceparentHeader, tracestateHeader, transport) {
  ...
  if (traceparent.isSampled === true) {
    sampler = this.agent.sampler.remoteParentSampled
  } else if (traceparent.isSampled === false) {
    sampler = this.agent.sampler.remoteParentNotSampled
  }
  this._calculatePriority(sampler, tracestate)
}
...
const _dtDefineAttrsFromTraceData = function _dtDefineAttrsFromTraceData(data, transport) {
  ...
  if (data.pr) {
    ...
    const sampler = data?.sa ? this.agent.sampler.remoteParentSampled : this.agent.sampler.remoteParentNotSampled
 ...
 * the given sampler will apply a sampling decision
 * to the transaction, which assigns `priority` and updates `sampled`
 * in respect to this sampling decision.
 * @param {Sampler} sampler the sampler to use, defaults to `agent.sampler.root`
 * @param {Tracestate} [tracestate] needed if sampler is an `AdaptiveSampler`
 */
Transaction.prototype._calculatePriority = function _calculatePriority(sampler = null, tracestate = null) {
  if (this.priority === null) {
    if (!sampler) {
      // If no sampler supplied, assume transaction
      // is from current service, so use root sampler
      sampler = this.agent.sampler.root
    }
