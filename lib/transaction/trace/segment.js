 ...
 *  'mysql', etc).
 */
function TraceSegment({ config, name, collect, traceStacks, root, isRoot = false }) {
  this.isRoot = isRoot
  this.root = root
  this.name = name
...
TraceSegment.prototype.getSpanContext = function getSpanContext() {
  const spansEnabled = this.config.distributed_tracing.enabled && this.config.span_events.enabled
...
TraceSegment.prototype.probe = function probe(action) {
  this.traceStacks.probe(action, { segment: this.name })
}
