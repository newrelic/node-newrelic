  ...
  while (toProcess.length > 0) {
    ...
    if (children.length === 0) {
      ...
      if (parent) {
        ...
        // if we have a parent and it has children to process, do the thing
...
function merge(first, second) {
  ...
  var secondCounter = 0
  var currInterval = (
    first[firstCounter][0] < second[secondCounter][0]
      ? first[firstCounter++]
      : second[secondCounter++]
  ).slice()
  while (firstCounter < first.length && secondCounter < second.length) {
    var next = first[firstCounter][0] < second[secondCounter][0]
      ? first[firstCounter++]
      : second[secondCounter++]
    if (next[0] <= currInterval[1]) {
      currInterval[1] = Math.max(next[1], currInterval[1])
    } else {
      res[resCounter++] = currInterval
      currInterval = next
    }
  }
  if (firstCounter === first.length) {
    if (second[secondCounter][0] <= currInterval[1]) {
      currInterval[1] = Math.max(second[secondCounter++][1], currInterval[1])
    }
    res[resCounter++] = currInterval
    for (;secondCounter < second.length; ++secondCounter) {
      res[resCounter++] = second[secondCounter]
    }
  } else {
    if (first[firstCounter][0] <= currInterval[1]) {
      currInterval[1] = Math.max(first[firstCounter++][1], currInterval[1])
    }
    res[resCounter++] = currInterval
    for (;firstCounter < first.length; ++firstCounter) {
      res[resCounter++] = first[firstCounter]
    }
  }
