 ...
 * match it. To properly name apdex metrics during metric recording, it's also
 * necessary to copy the transaction's partial name. And finally, marking the
 * trace segment as being a web segment copies the original URL's query
 * parameters onto the segment, as well as onto the transaction trace.
 *
 * @param {string}      rawURL      The URL, as it came in, for parameter
 *                                  extraction.
 * @param {Transaction} transaction The already-named transaction.
 */
TraceSegment.prototype.markAsWeb = function (rawURL, transaction) {
  // transaction scope and web segment name must match
  this.name = transaction.scope;
  // partialName is used to name apdex metrics when recording
  this.partialName = transaction.partialScope;

  // don't replace any existing segment or trace parameters, but do add to them
  var params = web.getParametersFromURL(rawURL);
  util._extend(this.parameters, params);
  util._extend(this.trace.parameters, params);
};
