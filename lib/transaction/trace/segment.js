...
/**
 * Adds SpanEvent event to TraceSegment unless the size limit has been reached
 *
 * @param {SpanEvent} spanEvent to add to TraceSegment
 * @returns {boolean} if span event was added or not
 */
TraceSegment.prototype.addSpanEvent = function addSpanEvent(spanEvent) {
  if (this.spanEvents.length === 100) {
    this.logger.trace({ spanEvent }, 'Span event limit reached. Not adding new event.')
    return false
  }
  this.spanEvents.push(spanEvent)
  return true
}
