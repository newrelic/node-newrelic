 ...
 * for. This is cached, so really on run once per trace for all segments in the tree.
 *
 * Uncollected segments are never added to the trace's segment tree (see
 * `tracer.createSegment`), so `_computeTotalTime` can never populate their
 * `_exclusiveDuration` — recomputing on every call would just waste time
 *  re-walking the whole tree for nothing. The root is the one exception:
 * it's always in the tree regardless of its own `_collect`, which instead
 * reflects `transaction.collect` (whether the detailed trace is kept).
 *
 *
...
function getExclusiveDurationInMillis(trace) {
  if (this._exclusiveDuration == null && (this._collect || this === trace.root)) {
