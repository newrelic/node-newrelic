...
class ExclusiveCalculator {
    ...
    while (this.toProcess.length) {
      ...
      if (children.length === 0) {
        ...
        if (this.parentStack.length) {
          this.finishChild(segment.timer.toRange())
        }
      } else {
  ...
  /**
   * Updates the immediate parent in the parent stack that a child has
   ...
   *
   * @param {Array} childRange An array of start and end time for a segment
   */
  finishChild(childRange) {
...
function merge(first, second) {
  ...
  var firstIdx = 0
  var secondIdx = 0
  // N.B. this is destructive, it will be updating the end times for the 
