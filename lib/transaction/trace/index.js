...
function Trace(transaction) {
  ...
  this.root = new Segment(
    this.transaction.agent.config,
    'ROOT',
    transaction.collect,
    transaction.traceStacks
  )
  this.root.start()
...
Trace.prototype.add = function add(config, childName, collect, traceStacks) {
  return this.root.add(config, childName, collect, traceStacks)
}
