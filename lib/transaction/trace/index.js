...
Trace.prototype.addAttribute = function addAttribute(key, value) {
  if (this.attributes.isValidLength(key)) {
    ...
    logger.warn(
      'Length limit exceeded for attribute name, not adding to transaction trace: ' + key
    )
  }
}
...
Trace.prototype._filterAttributes = function _filterAttributes(key) {
  var attFilter = this.filter
  return DESTINATIONS.filter(function testDest(dest) {
    return attFilter.test(dest, key)
  })
}
...
Trace.prototype.addCustomAttribute = function addCustomAttribute(key, value) {
  if (!this.custom.isValidLength(key)) {
    return logger.error(
      'Length limit exceeded for custom attribute name, not adding to trace:' + key
    )
  }
...
Trace.prototype.generateJSON = function generateJSON(callback) {
  var attributes = {
    agentAttributes: copy.shallow(this.attributes.get('transaction_tracer')),
    userAttributes: this.custom.get('transaction_tracer'),
    intrinsics: this.intrinsics
  }
  delete attributes.agentAttributes['request.uri']
