...
/**
 * Merges an array of `[start, end]` time ranges, sorted by start time, into a
 * single array of non-overlapping ranges. Unlike `_mergeRanges`, this takes a
 * single already start-sorted array and merges it in one linear pass, rather
 * than merging two sorted arrays together — used to combine the ranges of an
 * arbitrary number of children in O(m log m) instead of the O(k^2) that
 * results from folding `_mergeRanges` over each child one at a time.
 *
 * **Note:** This function is destructive — it mutates end times of elements
 * in the input array when merging overlapping intervals.
 *
 * @private
 * @param {Array} ranges - A start-sorted array of `[start, end]` time ranges.
 * @returns {Array} A new array of merged, non-overlapping time ranges.
 */
Trace.prototype._mergeSortedRanges = function _mergeSortedRanges(ranges) {
  if (!ranges.length) {
    return ranges
  }
...
Trace.prototype._computeTotalTime = function _computeTotalTime() {
  ...
  for (let i = segmentTree.length - 1; i >= 0; i--) {
    ...
    // Merge all direct children's propagated ranges. Collecting every child's
    // ranges and sorting once, rather than folding `_mergeRanges` over each
    // child in turn, avoids quadratic behavior for segments with many children
    const allRanges = []
    for (const child of children) {
      if (child._ranges) {
        for (const range of child._ranges) {
    ...
    const childPairs = this._mergeSortedRanges(allRanges)
