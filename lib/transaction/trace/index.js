...
Trace.prototype.generateSpanEvents = function generateSpanEvents() {
  ...
  if (
    ...
    while (toProcess.length) {
      ...
      // TODO: Cleanup creation/usage of aggregator code
      // Ideally, this code doesn't need to know/care different aggregator
      // types exist.
      if (
        config.feature_flag.streaming_spans &&
        config.mtb.endpoint
      ) {
        const streamingSpanEventAggregator = this.transaction.agent.streamingSpanEventAggregator
        streamingSpanEventAggregator.addSegment(
          segment, segmentInfo.parentId, segmentInfo.isRoot
        )
      } else {
