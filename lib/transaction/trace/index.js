...
Trace.prototype.end = function end() {
  ...
  if (config.span_events.enabled && config.feature_flag.distributed_tracing) {
    ...
    var segments = [{
      ...
      grandparentId: this.transaction.parentId || null,
      segment: this.root
    }]

    while (segments.length) {
      var segmentInfo = segments.pop()
      ...
      if (!insertionFailed) {
        insertionFailed = !this.transaction.agent.spans.addSegment(
          segment,
      ...
      segments = segments.concat(
        segment.getChildren().map((child) => {
          return {
            parentId: segment.id,
            grandparentId: segmentInfo.parentId,
            segment: child
          }
        })
      )
