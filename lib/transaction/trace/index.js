...
Trace.prototype.generateSpanEvents = function generateSpanEvents() {
  ...
  if (
    ...
    for (let i = 0; i < children.length; ++i) {
      if (i === 0) {
        // At the point where 'host.displayName' is available we only 
        // have a root span.  Adding attribute to first non-root span here.
        const traceAttributes = this.attributes.get(DESTINATIONS.TRANS_COMMON)
        if (traceAttributes['host.displayName']) {
          children[i].addSpanAttribute(
            'host.displayName',
            traceAttributes['host.displayName']
          )
        }
      }
