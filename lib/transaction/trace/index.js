...
const { mergeRanges } = require('./exclusive-time-calculator')
...
Trace.prototype._updateExclusiveDurationForSegment = function _updateExclusiveDurationForSegment() {
}
 ...
 * up the tree, mirroring the ExclusiveCalculator algorithm.
 *
 * @private
...
Trace.prototype._computeTotalTime = function _computeTotalTime() {
  ...
  for (let i = segmentTree.length - 1; i >= 0; i--) {
    ...
    for (const child of children) {
      childPairs = mergeRanges(childPairs, child._ranges)
      child._ranges = null
    }
    ...
    node._ranges = mergeRanges(childPairs, [segment.timer.toRange()])
