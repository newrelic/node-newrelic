...
class PartialTrace {
  ...
  addSpan({ span, isEntry }) {
    ...
    if (spanLinks && spanLinks.length > 0) {
      // remove non-intrinsic attributues for essential and compact modes
  ...
  reparentSpanLinks(span, spanLinksToReparent) {
    ...
    for (const link of spanLinksToReparent) {
      if (span.spanLinks.length === 100) {
        logger.trace({ spanLinksToReparent }, 'Span links limit reached. Not moving new link.')
        // add supportability metric here
        return
      }
  ...
  applyCompaction(span) {
    ...
    if (meta.spanLinks.length > 0) {
      this.removeNonIntrinsicsAttrs(meta.spanLinks)
