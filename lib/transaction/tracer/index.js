...
function createSegment(name, recorder, _parent) {
  ...
  const segment = parent.add({ config: this.agent.config, name, collect, traceStacks: transaction.traceStacks, root: transaction.trace.root })
...
function bindFunction(handler, segment, full) {
  ...
  const ctx = {}
  if (segment?.segment) {
    ctx.segment = segment?.segment
    ctx.transaction = segment?.transaction
  } else {
    ctx.segment = segment || this.getSegment()
    ctx.transaction = this.getTransaction()
  }
