...
function bindFunction(handler, segment, full) {
  ...
  function wrapped() {
    ...
    try {
      ...
      if (!err[ERROR_TRANSACTION]) err[ERROR_TRANSACTION] = active && active.transaction
      throw err
    } finally {
...
function wrapFunctionNoSegment(original) {
...
function wrapFunctionFirst(name, recorder, original) {
  ...
  function wrappedFunction() {
    if (!tracer.getTransaction() || typeof arguments[0] !== 'function') {
...
function wrapFunction(name, recorder, original, wrapper, resp) {
  if (!original || !wrapper) return original
  var tracer = this
