...
var ERROR_TRANSACTION = '__NR_transaction'
...
function bindFunction(handler, segment, full) {
  ...
  function wrapped() {
    ...
    try {
      ...
      if (err instanceof Error && active && !err[ERROR_TRANSACTION]) {
        try {
          Object.defineProperty(err, ERROR_TRANSACTION, {
            enumerable: false,
            configurable: true,
            writable: true,
          ...
          logger.debug("Could not annotate error object: %s", _err)
        }
      }
...
function error(err) {
  if (!err) return
  var transaction = err[ERROR_TRANSACTION] || this.getTransaction()
  try {
    err[ERROR_TRANSACTION] = null
  } catch (err) {
    logger.debug('Encountered immutable error when trying to assign transaction: %s',
                 err.message)
  }
