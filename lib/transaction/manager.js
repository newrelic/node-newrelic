...
var getAgentName = function getAgentName(agent) {
    ...
    if (agent.config && agent.config.app_name && agent.config.app_name.length) agentID = agent.config.app_name[0];
...
var manager = {
   ...
   * @param {number} height How high up the call stack to push the annotation.
   *
   * @returns {Transaction} Ready-to-use transaction (with its own (currently
   *                        unused) timer).
   */
  create : function (agent, height) {
    var blank = new Transaction(agent);

    if (!height) height = 2;

    callstack.annotateCaller(blank, height);
  ...
  find : function (agent) {
    return callstack.findAnnotation();
  },
