...
class TraceContext {
  ...
  acceptTraceContextPayload(traceparentHeader, tracestateHeader) {
    ...
    let parsedParent
    try {
      parsedParent = Traceparent.fromHeader(traceparentHeader)
    } catch (error) {
    ...
    // TODO: implement decision algorithm ~ jsumners
    // if (this.transaction.traceparentSampledDecision() === 2.0) {
    //   traceContextData.sampled = true
    //   traceContextData.priority = 2.0
    // }
    ...
    let parsedState
    try {
      parsedState = Tracestate.fromHeader({ header: tracestateHeader, agent: this.transaction.agent })
    } catch (error) {
    ...
    this._traceStateRaw = parsedState.toString()

    // These need to be kept to be added to root span events as an attribute
    this.tracingVendors = parsedState.vendors

    if (parsedState.intrinsics && parsedState.intrinsics.version !== TracestateIntrinsics.NR_TRACESTATE_VERSION) {
      logger.trace(
    ...
    if (parsedState.intrinsics?.isValid === true) {
