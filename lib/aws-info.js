...
function awsQuery(key, agent, callback) {
  ...
  var req = http.get(url, function awsRequest(res) {
    ...
    function respond(data) {
      ...
      if (!valid) {
        ...
        logger.info('Failed to retrieve valid response for AWS attribute: %s' + key, data)
        data = null
      }
      callback(data)
    }
  ...
  req.setTimeout(1000, function awsTimeout() {
    logger.info('Request for AWS attribute ' + key + ' timed out')
    callback(null)
  })
  req.on('error', function awsError(err) {
    logger.info('There was an error in requesting AWS attribute ' + key + ': %s'
        , err.message)
    callback(null)
  })
}
