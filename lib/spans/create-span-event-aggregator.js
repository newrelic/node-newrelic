...
function createSpanEventAggregator(config, collector, metrics) {
  ...
  if (config.feature_flag.infinite_tracing && config.infinite_tracing.trace_observer) {
    ...
    // If logic happened prior, could merely check for existance of trace_observer.

    const traceObserverUrl = config.infinite_tracing.trace_observer
...
function validateInfiniteTracing(metrics, traceObserverUrl) {
  ...
  if (!isValidUrl(traceObserverUrl)) {
    ...
    logger.warn('Infinite tracing disabled: `trace_observer` must be a valid url')
