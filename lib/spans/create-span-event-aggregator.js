...
function createSpanEventAggregator(config, collector, metrics) {
  ...
  if (config.feature_flag.infinite_tracing 
    ...
    shouldCreateStreaming = validateInfiniteTracing(config)
...
function validateInfiniteTracing(config) {
  ...
  if (!validateHostName(config.infinite_tracing.trace_observer.host.trim())) {
    logger.warn('Infinite tracing disabled: invalid infinite_tracing.trace_observer.host value')

    return false
  }

  if (!validatePortValue(config.infinite_tracing.trace_observer.port)) {
...
function validatePortValue(port) {
  ...
  return !isNaN(port.trim())
}

function getTraceObserverEndpoint(config) {
  /* eslint-disable-next-line */
  return `${config.infinite_tracing.trace_observer.host.trim()}:${String(config.infinite_tracing.trace_observer.port).trim()}`
}
