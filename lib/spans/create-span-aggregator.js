...
const SpanAggregator = require('./span-aggregator')
const StreamingSpanAggregator = require('./streaming-span-aggregator')
const NAMES = require('../metrics/names').INFINITE_TRACING

function createSpanAggregator(config, agent) {
...
function createStreamingAggregator(config, agent) {
  ...
  // the send method on StreamingSpanAggregator is a no-op
  const opts = {
    periodMs: 1000,
    limit: 50000,
  ...
  return new StreamingSpanAggregator(opts, agent)
}
...
function createStandardAggregator(config, agent) {
  ...
  return new SpanAggregator(opts, agent)
}

module.exports = createSpanAggregator
