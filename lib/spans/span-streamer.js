...
const NO_STREAM_WARNING = 'Attempting to stream spans before connection created.'
...
class SpanStreamer {
  ...
  write(span) {
    ...
    // If not connected, discard the span
    if (!this.stream) {
    ...
    if (!this._writable) {
      if (this.spans.length <= this.queue_size) {
        ...
        logger.trace('Span stream queue size: %s', this.spans.length)
        return
      }
