...
/**
 * Specialized span class for use with infinite streaming.
...
class StreamingSpan {
  ...
  static fromSegment({ segment, transaction, parentId = null, isRoot = false, isEntry = false }) {
    ...
    let span = null
    if (StreamingHttpSpan.isHttpSegment(segment)) {
      span = new StreamingHttpSpan(traceId, agentAttributes, customAttributes)
    } else if (StreamingDatastoreSpan.isDatastoreSegment(segment)) {
      span = new StreamingDatastoreSpan(traceId, agentAttributes, customAttributes)
    } else {
      span = new StreamingSpan(traceId, agentAttributes, customAttributes)
    }
 ...
 * @class
 */
class StreamingHttpSpan extends StreamingSpan {
  /**
   * @param {*} traceId TraceId for the Span.
   * @param {object} agentAttributes Initial set of agent attributes.
   * Must be pre-filtered and truncated.
 ...
 * @class
 */
class StreamingDatastoreSpan extends StreamingSpan {
  /**
   * @param {*} traceId TraceId for the Span.
   * @param {object} agentAttributes Initial set of agent attributes.
   * Must be pre-filtered and truncated.
