 ...
 * @class
 *
 * @property {Attributes} customAttriutes User attributes that were added to
 * the message before it was stored.
 * @property {Attributes} attributes Agent attributes that the New Relic agent
 * has determined need to be present.
...
class SpanLink {
   ...
   */
  constructor({ link, spanContext, timestamp = 0 } = {}) {
    this.userAttributes = new Attributes({ scope: 'segment' })
    this.agentAttributes = new Attributes({ scope: 'segment' })
    this.intrinsics = Object.create(null)

    if (!link || !spanContext) {
      throw Error('cannot create span link without required data')
    }
