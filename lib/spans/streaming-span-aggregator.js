...
const StreamingSpan = require('./streaming-span')
 ...
 *
 * @event StreamingSpanAggregator#started
 */

/**
 * Indicates that span streaming has finished.
 *
 * @event StreamingSpanAggregator#stopped
 */
 ...
 */
class StreamingSpanAggregator extends Aggregator {
  ...
  toString() {
    return 'StreamingSpanAggregator'
  }

  /**
   * Start streaming spans to the collector.
   *
   * @fires StreamingSpanAggregator#started
   */
  start() {
    if (this.started) {
      return
    ...
    logger.trace('StreamingSpanAggregator starting up')
   ...
   *
   * @fires StreamingSpanAggregator#stopped
   */
  stop() {
    if (!this.started) {
      return
    ...
    logger.trace('StreamingSpanAggregator stopping')
  ...
  addSegment({ segment, transaction, parentId, isRoot, isEntry }) {
    ...
    const span = StreamingSpan.fromSegment({ segment, transaction, parentId, isRoot, isEntry })
...
module.exports = StreamingSpanAggregator
