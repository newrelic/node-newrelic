...
function getProcessorStats(callback) {
  ...
  if (platform.match(/darwin/i)) {
    ...
    logger.info('Unknown platform: ' + platform + ', could not retrieve processor info')
    callback(processorStats)
  }
}
...
function getMemoryStats(callback) {
  if (platform.match(/darwin/i)) {
    ...
    logger.info('Unknown platform: ' + platform + ', could not retrieve memory info')
    callback(null)
  }
}
...
function getKernelVersion(callback) {
  if (platform.match(/darwin/i)) {
    ...
    logger.info('Unknown platform' + platform + ', could not read kernel version')
    callback(null)
  }
}
...
function getDockerContainerId(agent, callback) {
  if (!platform.match(/linux/i)) {
    logger.info('Platform is not a flavor of linux, omitting docker info')
    callback(null)
  } else {
    readProc('/proc/self/cgroup', function(data) {
...
function getSysctlValue(names, callback) {
  ...
  names.forEach(function sysctlName(name) {
    ...
    function respond(err, stdout, stderr) {
      if (returned) return
      if (err) {
        logger.info('Error when trying to run: sysctl -n ' + name + ': %s', err.message)
        callback(null)
        returned = true
      } else if (!stderr) {
        callback(stdout)
      ...
      if (++ran === names.length && !returned) {
        logger.info('No sysctl info found for names: ' + names.toString())
        callback(null)
      }
    }
  })
