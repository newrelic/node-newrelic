 ...
 */
function listPackages(root, packages, cb) {
  // listPackages(root, cb)
 ...
 * @param {function}  cb          - Callback to send deps to.
 *
 * @return {Array} List of dependencies.
 */
function listDependencies(root, children, visited, cb) {
  // listDependencies(root, cb)
  if (typeof children === 'function') {
    cb = children
    children = []
    visited = Object.create(null)
  }
  // listDependencies(root, {children|visited}, cb)
  ...
  function forEachEntry(entry, cb) {
    ...
    fs.realpath(candidate, function realPathCb(err, realCandidate) {
      ...
      ], function onRecurseListComplete(err) {
        if (err) {
          logger.debug(err, 'Failed to list dependencies in %s', realCandidate)
        }
        cb()
      })
    })
 ...
 *  appropriate names.
 */
function getLocalPackages(start, cb) {
  var packages = []
  var dependencies = []
  var candidate = start
