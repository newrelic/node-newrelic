 ...
 *                        backstop.
 */
function MetricNormalizer(config) {
  if (!config) throw new Error ("Metric normalizer requires configuration.");
  this.config   = config;
  this.rules    = [];
}
...
MetricNormalizer.prototype.load = function (json) {
  if (json) {
    logger.debug("Received %d metric normalization rule(s)", json.length);
...
MetricNormalizer.prototype.normalize = function (path) {
  ...
  if (this.config.enforce_backstop) {
