...
class SetupMetrics extends SetupSignal {
  constructor({ agent, logger = defaultLogger } = {}) {
    ...
    const { config } = agent
    let exportInterval = config.opentelemetry.metrics.export_interval
    ...
    // Our OTEL metrics spec dictates that we forward OTEL diagnostics logging
    // to our agent logger. It does not specify what levels should be targeted.
    // We opt to do this only when debug logging has been enabled.
    if (logger.debugEnabled() === true || logger.traceEnabled() === true) {
      const DiagLogger = require('./diag-logger.js')
      const { diag, DiagLogLevel } = require('@opentelemetry/api')
      diag.setLogger(new DiagLogger({ logger }), DiagLogLevel.DEBUG)
    }
