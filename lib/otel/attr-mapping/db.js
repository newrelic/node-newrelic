...
function dbMapper({ segment }) {
  const { mapping: dbPortMapping } = getMapping({ key: 'port', segment })
  const { mapping: dbServerMapping } = getMapping({ key: 'server', segment })
  const { mapping: dbNameMapping } = getMapping({ key: 'name', segment })
  const { mapping: dbSystemMapping } = getMapping({ key: 'system', segment })
  const { mapping: dbQueryMapping } = getMapping({ key: 'query' })
  return {
    ...dbPortMapping,
    ...dbServerMapping,
    ...dbNameMapping,
