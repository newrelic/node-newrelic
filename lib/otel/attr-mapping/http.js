...
const { attrMapper } = require('./utils')
...
const getMapping = attrMapper.bind(attrMappings)

function rpcMapper({ segment, transaction }) {
  const { mapping: statusMapping } = getMapping({ key: 'grpcStatusCode', segment, transaction })
  const { mapping: systemMapping } = getMapping({ key: 'rpcSystem', segment })
  const { mapping: rpcMethodMapping } = getMapping({ key: 'rpcMethod', transaction })
  return {
    ...rpcMethodMapping,
    ...statusMapping,
    ...systemMapping
...
function serverMapper({ segment, transaction }) {
  ...
  const { mapping: methodMapping } = getMapping({ key: 'method', transaction })
  const { mapping: statusCodeMapping } = getMapping({ key: 'statusCode', transaction })
  const { mapping: statusTextMapping } = getMapping({ key: 'statusText', transaction })
  const { mapping: serverPortMapping } = getMapping({ key: 'port', segment })
  const { mapping: httpRouteMapping } = getMapping({ key: 'route', segment, transaction })
  const { mapping: httpHostMapping } = getMapping({ key: 'host', segment, transaction })
  return {
    ...httpHostMapping,
    ...httpRouteMapping,
    ...methodMapping,
...
function clientMapper({ segment }) {
  const { mapping: statusCodeMapping } = getMapping({ key: 'statusCode', segment })
  const { mapping: statusTextMapping } = getMapping({ key: 'statusText', segment })
  const { mapping: clientHostMapping } = getMapping({ key: 'clientHost', segment })
  const { mapping: clientUrlMapping } = getMapping({ key: 'clientUrl', segment })
  const { mapping: clientPortMapping } = getMapping({ key: 'clientPort', segment })
  return {
    ...clientHostMapping,
    ...clientPortMapping,
    ...clientUrlMapping,
