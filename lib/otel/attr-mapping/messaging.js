...
const attrMappings = {
  ...
  destination: {
    ...
    mapping({ transaction }) {
      ...
      const isHighSecurity = transaction.agent.config.high_security ?? false
      return (value) => {
  ...
  operation: {
    attrs: [constants.ATTR_MESSAGE_OPERATION, constants.ATTR_MESSAGING_DESTINATION_KIND]
  },
  port: {
...
function consumerMapper({ transaction }) {
  const { mapping: destMapping } = getMapping({ key: 'destination', transaction })
  const { mapping: portMapping } = getMapping({ key: 'port', segment: transaction.baseSegment })
  const { mapping: serverMapping } = getMapping({ key: 'server', segment: transaction.baseSegment })
  const { mapping: rmqRoutingKeyMapping } = getMapping({ key: 'rmqRoutingKey', transaction })
  return {
    ...destMapping,
    ...portMapping,
...
function producerMapper({ segment }) {
  const { mapping: conversationIdtMapping } = getMapping({ key: 'conversationId', segment })
  const { mapping: portMapping } = getMapping({ key: 'port', segment })
  const { mapping: serverMapping } = getMapping({ key: 'server', segment })
  const { mapping: rmqRoutingKeyMapping } = getMapping({ key: 'rmqRoutingKey', segment })
  return {
