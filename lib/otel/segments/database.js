...
const { getMapping } = require('../attr-mapping/db')
...
module.exports = function createDbSegment(agent, otelSpan) {
  ...
  const { value: system } = getMapping({ key: 'system', span: otelSpan })
...
function parseStatement(config, otelSpan, system) {
  let { value: table } = getMapping({ key: 'table', span: otelSpan })
  let { value: operation } = getMapping({ key: 'operation', span: otelSpan })
  let { value: statement } = getMapping({ key: 'query', span: otelSpan })
