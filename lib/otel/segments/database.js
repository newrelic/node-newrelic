...
module.exports = function createDbSegment(agent, otelSpan, rule) {
  ...
  if (transformationRule) {
    ...
    debugger
    const name = transformTemplate(segmentTransformation.name.template, parsedStatement)
    // TODO: need to abstract common things but I htink this  is good to go
    // I also need to flesh out specific rules based on the db.system for redis/memcached
    // also need to build out 1.17.0 rules as all otel js uses that atm as far as i can see
    const segment = agent.tracer.createSegment({
      id: otelSpan?.spanContext()?.spanId,
      name,
      recorder: getRecorder({ operation: name.startsWith('Datastore/operation'), parsed: parsedStatement, system }),
