...
function setName(otelSpan) {
  ...
  // TODO: how to determine when to set topic for gcp-pubsub? It's not in the attributes
  const destKind = otelSpan.attributes[ATTR_MESSAGING_DESTINATION_KIND] || 'Unknown'
  const destination = otelSpan.attributes[ATTR_MESSAGING_DESTINATION_NAME] || otelSpan.attributes[ATTR_MESSAGING_DESTINATION] || 'Unknown'
