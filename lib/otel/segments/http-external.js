...
module.exports = function createHttpExternalSegment(agent, otelSpan) {
  ...
  const { value: method } = getMapping({ key: 'method', span: otelSpan })
  const { value: host = UNKNOWN } = getMapping({ key: 'clientHost', span: otelSpan })

  const { value: url } = getMapping({ key: 'clientUrl', span: otelSpan })
