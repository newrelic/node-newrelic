...
module.exports = function createDbSegment(agent, otelSpan, rule) {
  ...
  const systemAttrKey = segmentTransformation?.type
  let system = otelSpan.attributes[systemAttrKey]
  // Use the normalized value from matched attribute conditions if available.
  // When a rule uses attribute_conditions with {expected, value} format,
  // the `value` is stored in rule.matchedConditionValues for use here.
  if (rule?.matchedConditionValues && systemAttrKey) {
    const normalizedValue = rule.matchedConditionValues[systemAttrKey]
 ...
 * @param {string} params.system `db.system.name` value of otel span
