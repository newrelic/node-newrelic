 ...
 * be "mongodb" by the OTEL instrumentation. Each condition may also be an
 * object of the form `{ expected: string[], value: string }` to match
 * multiple attribute values to a single normalized display name. For
 * example, `{ db.system: { expected: ["sqlite3", "better-sqlite3"],
 * value: "sqlite" } }` matches either `sqlite3` or `better-sqlite3` and
 * resolves to the normalized value `sqlite`.
  ...
  matches(span) {
    ...
    for (const [key, value] of Object.entries(this.#requiredConditions)) {
      if (Array.isArray(value)) {
        if (value.includes(span.attributes[key])) {
          attrConditions += 1
        }
      } else if (typeof value === 'object' && value !== null && 'expected' in value && 'value' in value) {
        // Support {expected: [...], value: "normalized"} form for matching
        // The span's attribute must match one of the expected values
        if (Array.isArray(value.expected) && value.expected.includes(span.attributes[key])) {
          attrConditions += 1
        }
      } else if (span.attributes[key] === value) {
        attrConditions += 1
      }
    }
