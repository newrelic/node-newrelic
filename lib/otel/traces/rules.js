...
class Rule {
  ...
  /**
   * Returns matched attribute condition values for use in segment transformation.
   * When an attribute condition used {expected, value} format, the value is stored here.
   * @returns {object} Map of attribute key to its resolved (possibly normalized) value.
   */
  get matchedConditionValues() {
    return this.#matchedConditionValues
  }
  ...
  matches(span) {
    ...
    for (const [key, value] of Object.entries(this.#requiredConditions)) {
      ...
      if (Array.isArray(value)) {
        if (value.includes(spanValue)) {
          attrConditions += 1
          this.#matchedConditionValues[key] = spanValue
        }
      } else if (typeof value === 'object' && value !== null && Array.isArray(value.expected)) {
        if (value.expected.includes(spanValue)) {
          attrConditions += 1
          this.#matchedConditionValues[key] = value.value
        }
      } else if (spanValue === value) {
        attrConditions += 1
        this.#matchedConditionValues[key] = spanValue
      }
    }
