 ...
 * must match a value from an array, a scalar, or an object. For example, if the
 * `required_attribute_keys` list contains `"db.system"`, this object may
 * contain a field named `db.system` with any of:
 * - An array: `["mongodb"]` — matches if the span attribute is in the array.
 * - A scalar: `"mongodb"` — matches exact value.
 * - An object: `{"expected": ["sqlite3", "better-sqlite3"], "value": "sqlite"}` —
 *   matches if the span attribute is in `expected`, and overwrites the span
 *   attribute with the canonical `value` for downstream segment naming.
...
class Rule {
  ...
  matches(span) {
    ...
    for (const [key, condition] of Object.entries(this.#requiredConditions)) {
      if (Array.isArray(condition)) {
        if (condition.includes(span.attributes[key])) {
          attrConditions += 1
        }
      } else if (typeof condition === 'object' && condition.expected !== undefined) {
        // Object form: { expected: [...], value: "canonical" }
        // Matches if span attribute is in expected list;
        // writes canonical value back to span for downstream use.
        if (condition.expected.includes(span.attributes[key])) {
          span.attributes[key] = condition.value
          attrConditions += 1
        }
      } else {
        // Scalar form: exact match
        if (span.attributes[key] === condition) {
          attrConditions += 1
        }
      }
    }
