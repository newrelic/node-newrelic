...
module.exports = class NrSpanProcessor {
  ...
  reconcileAttributes({ segment, span, transaction }) {
    ...
    } else if (span.attributes[ATTR_MESSAGING_SYSTEM]?.toLowerCase().includes('sqs')) {
      // should be 'aws.sqs' but I've seen 'AmazonSQS' and 'aws_sqs' referenced in otel docs
      // see: https://github.com/open-telemetry/opentelemetry-js-contrib/blob/b520d048465d9b3dfdf275976010c989d2a78a2c/plugins/node/opentelemetry-instrumentation-aws-sdk/src/services/sqs.ts#L63C37-L63C44
  ...
  reconcileDbAttributes({ segment, span }) {
    ...
    if (span.attributes[ATTR_DB_SYSTEM] === DB_SYSTEM_VALUES.DYNAMODB) {
      const region = span.attributes['aws.region']
  ...
  reconcileLambdaAttributes({ segment, span }) {
    // see https://github.com/open-telemetry/opentelemetry-specification/blob/v1.7.0/specification/trace/semantic_conventions/faas.md#example
    // see https://github.com/open-telemetry/opentelemetry-js-contrib/tree/main/plugins/node/opentelemetry-instrumentation-aws-lambda#semantic-conventions 
    // Add entity linking attrs: cloud.resource_id and cloud.platform
    const region = span.attributes['faas.invoked_region'] ?? span.attributes['aws.region']
    const accountId = span.attributes['cloud.account.id'] ?? this.agent.config.cloud.aws.account_id
  ...
  reconcileSQSAttributes({ segment, span }) {
    // see: https://github.com/open-telemetry/opentelemetry-specification/blob/v1.7.0/specification/trace/semantic_conventions/messaging.md#messaging-attributes
    // see: https://github.com/open-telemetry/opentelemetry-js-contrib/tree/main/plugins/node/opentelemetry-instrumentation-aws-sdk
    // Add entity linking attrs: cloud.account.id, cloud.region, messaging.destination.name, messaging.system 
    if (this.agent.config.cloud.aws.account_id) {
      segment.addAttribute('cloud.account.id', this.agent.config.cloud.aws.account_id)
    }

    const mapper = {
      ['aws.region']: (value) => segment.addAttribute('cloud.region', value),
      [ATTR_MESSAGING_DESTINATION]: (value) => segment.addAttribute('messaging.destination.name', value),
      [ATTR_MESSAGING_SYSTEM]: () => segment.addAttribute('messaging.system', 'aws_sqs'),
    }
