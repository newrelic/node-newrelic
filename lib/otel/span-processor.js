...
module.exports = class NrSpanProcessor {
  ...
  handleError(segment, transaction, span) {
    ...
    errorEvents.forEach((err) => {
      if (err) {
        const msg = err.attributes['exception.message'] || err.attributes['exception.type'] || `Error from ${segment.name}`
        const stack = err.attributes['exception.stacktrace']
        const error = new OtelError(msg, stack)
