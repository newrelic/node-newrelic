...
module.exports = class NrSpanProcessor {
  ...
  reconcileAttributes({ segment, span, transaction }) {
    if (span.kind === SpanKind.SERVER) {
      ...
      this.reconcileHttpExternalAttributes({ segment, span, transaction })
    }
  }

  reconcileHttpExternalAttributes({ segment, span, transaction }) {
    for (const [prop, value] of Object.entries(span.attributes)) {
      ...
      if (key === ATTR_HTTP_REQUEST_METHOD || key === ATTR_HTTP_METHOD) {
        ...
        key = 'http.statusText'
        segment.addSpanAttribute(key, sanitized)
      } else if (key === ATTR_URL_SCHEME || key === ATTR_HTTP_URL) {
        sanitized = this.constructUrl(span)
        key = 'url'
      } else if (key === ATTR_URL_QUERY || key === ATTR_HTTP_TARGET) {
  ...
  constructUrl(span) {
    const scheme = span.attributes[ATTR_URL_SCHEME] || span.attributes[ATTR_HTTP_URL]?.split(':')[0]
    const host = span.attributes[ATTR_SERVER_ADDRESS] || span.attributes[ATTR_NET_PEER_NAME]
    let path = urltils.scrubAndParseParameters(span.attributes[ATTR_URL_PATH] || span.attributes[ATTR_HTTP_TARGET])
    path = urltils.obfuscatePath(this.agent.config, path)
  ...
  reconcileHttpAttributes({ segment, span, transaction }) {
    for (const [prop, value] of Object.entries(span.attributes)) {
      ...
      } else if (key === ATTR_HTTP_RESP_STATUS_CODE) {
