...
module.exports = class NrSpanProcessor {
  ...
  reconcileAttributes({ segment, span, transaction }) {
    ...
    if (span.attributes['faas.id']?.includes('aws:lambda')){ // client or server?
  ...
  reconcileDbAttributes({ segment, span }) {
    ...
    if (span.attributes[ATTR_DB_SYSTEM] === 'dynamodb') {
      ...
      let region = span.attributes['aws.region'] ?? 'unknown'
      let accountId = this.agent.config.cloud.aws.account_id ?? 'unknown'
      let collection = span.attributes['aws.dynamodb.table_names'] ?? 'unknown' // or is it span.attributes.collection? 
  ...
  reconcileLambdaAttributes({ segment, span }) {
    ...
    let region = span.attributes['cloud.region'] ?? 'unknown'
    let accountId = this.agent.config.cloud.aws.account_id ?? 'unknown'
    let functionName = span.attributes['faas.name'] ?? 'unknown'
