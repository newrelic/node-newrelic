...
module.exports = class NrSpanProcessor {
  ...
  reconcileDbAttributes({ segment, span }) {
    ...
    if (span.attributes[ATTR_DB_SYSTEM] === 'dynamodb') {
      ...
      //   from test/versioned/aws-sdk-v3/client-dynamodb.test.js:233
      const region = span.attributes['aws.region'] ?? 'unknown'
      const accountId = this.agent.config.cloud.aws.account_id ?? 'unknown'
      const collection = span.attributes['aws.dynamodb.table_names'] ?? 'unknown'
  ...
  reconcileLambdaAttributes({ segment, span }) {
    ...
    const region = span.attributes['cloud.region'] ?? span.attributes['faas.invoked_region'] ?? 'unknown'
    const accountId = this.agent.config.cloud.aws.account_id ?? 'unknown'
    const functionName = span.attributes['faas.name'] ?? span.attributees['faas.invoked_name'] ?? 'unknown'
  ...
  reconcileSQSAttributes({ segment, span }) {
    ...
    // Add entity linking attrs: messaging.system, cloud.region, cloud.account.id, messaging.destination.name
    segment.addAttribute('messaging.system', 'aws_sqs')
    const region = span.attributes['cloud.region'] ?? 'unknown'
    segment.addAttribute('cloud.region', region)
    const accountId = this.agent.config.cloud.aws.account_id ?? 'unknown'
    segment.addAttribute('cloud.account.id', accountId)
    const msgDestName = span.attributes['messaging.destination'] ?? 'unknown'
    segment.addAttribute('messaging.destination.name', msgDestName)
  }
}
