...
module.exports = class NrSpanProcessor {
  ...
  reconcileConsumerAttributes({ span, transaction }) {
    ...
    let serverAddress = attrs[ATTR_SERVER_ADDRESS]
    if (serverAddress) {
      if (urltils.isLocalhost(serverAddress)) {
        serverAddress = this.agent.config.getHostnameSafe(serverAddress)
      }
    ...
    const routingKey = attrs[ATTR_MESSAGING_RABBITMQ_DESTINATION_ROUTING_KEY]
    if (routingKey) {
      trace.attributes.addAttribute(DESTINATIONS.TRANS_COMMON, 'message.routingKey', routingKey)
      baseSegment.addAttribute('message.routingKey', routingKey)
    }
    ...
    if (destName) {
      ...
      baseSegment.addAttribute('message.queueName', destName)
    }
