...
module.exports = class NrSpanProcessor {
  ...
  reconcileHttpExternalAttributes({ segment, span }) {
    for (const [prop, value] of Object.entries(span.attributes)) {
      const { key, sanitized } = this.getSanitizedAttribute(prop, value, span)
      segment.addAttribute(key, sanitized)
    }
  }

  getSanitizedAttribute(prop, value, span) {
    let key = prop
    let sanitized = value
    if (key === ATTR_URL_SCHEME) {
      key = 'protocol'
    } else if (key === ATTR_SERVER_ADDRESS || key === ATTR_HTTP_HOST) {
      key = key === ATTR_SERVER_ADDRESS ? 'host' : 'hostname'
      if (urltils.isLocalhost(sanitized)) {
        sanitized = this.agent.config.getHostnameSafe(sanitized)
      }
    } else if (key === ATTR_HTTP_REQUEST_METHOD) {
      key = 'method'
    } else if (key === ATTR_SERVER_PORT) {
      key = 'port'
    } else if (key === ATTR_URL_PATH) {
      key = 'path'
      if (span.attributes[ATTR_URL_QUERY]) {
        sanitized = sanitized + '?' + span.attributes[ATTR_URL_QUERY]
      }
      // path includes URI path and query
      sanitized = urltils.obfuscatePath(this.agent.config, sanitized)
    }
