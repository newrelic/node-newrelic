...
If infinite tracing is configured, it creates a [StreamingSpan](https://github.com/newrelic/node-newrelic/blob/103044d25a968649bdb0e39df054757163cd986f/lib/spans/streaming-span-event.js#L111) and adds to the [StreamingSpanAggregator stream](https://github.com/newrelic/node-newrelic/blob/103044d25a968649bdb0e39df054757163cd986f/lib/spans/streaming-span-event-aggregator.js#L118).  
 ...
 * `reduced`:  it will create a [SpanEvent](https://github.com/newrelic/node-newrelic/blob/103044d25a968649bdb0e39df054757163cd986f/lib/spans/span-event.js#L237), and then decide whether or not the span should be kept.  If it is an entry point or LLM span, it will be kept.  If it is an exit span (e.g. `span.kind` of client), it will be kept if it has one attribute that is used to make an [entity relationship](https://github.com/newrelic/node-newrelic/blob/103044d25a968649bdb0e39df054757163cd986f/lib/spans/span-event.js#L21).
